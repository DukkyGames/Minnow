/**
 * Sidecar JSON vector index for semantic memory retrieval.
 * Persists under ~/.minnow/memory/vectors.json.
 */
...
/** Resolve vectors.json path. */
export function getVectorStorePath() {
  return path.join(getMemoryDir(), 'vectors.json');
}
 ...
 */
export function isVectorStoreCompatible(store, embeddingsConfig) {
 ...
 */
export async function upsertEntryVector(entryId, vector, meta = {}) {
...
export async function deleteEntryVector(entryId) {
...
/**
 * Rebuild the vector index for all memory entries.
 * @param {(text: string) => Promise<number[]>} embedOne
 * @param {Array<{ meta: object, body: string }>} entries
 * @param {{ model: string, backend: string, dim: number }} meta
 */
export async function reindexAllMemoryEntries(embedOne, entries, meta) {
  const next = {
    version: STORE_VERSION,
    model: meta.model,
...
export async function getEntryVector(entryId) {
