/**
 * Memory proposal queue persistence (~/.minnow/memory/proposals.json).
 */
...
function proposalsPath() {
  return path.join(getMemoryDir(), 'proposals.json');
}
 ...
 */
export async function addMemoryProposal(input) {
 ...
 * @returns {Promise<MemoryProposal[]>}
 */
export async function listMemoryProposals(status) {
  const cfg = await loadSynthesisConfig();
  const store = await loadStore();
 ...
 */
export async function getMemoryProposal(id) {
  const store = await loadStore();
 ...
 * @returns {Promise<MemoryProposal | null>}
 */
export async function updateMemoryProposalStatus(id, status) {
 ...
 * @returns {Promise<MemoryProposal | null>}
 */
export async function applyMemoryProposalEdits(id, edits) {
  const store = await loadStore();
  const row = store.proposals.find((p) => p.id === id);
  if (!row) return null;
...
export async function countPendingMemoryProposals() {
