/**
 * Model producer detection: slug, display name, and inline SVG logo for the
 * custom model picker. Path-prefix rules handle org/model ids; regex fallback
 * covers flat LM Studio ids (e.g. qwen3.5-9b, gemma-4-12b).
 */

export type ModelProducer = {
  slug: string;
  displayName: string;
  logoSvg: string | null;
};

/** Path-prefix aliases (first segment before `/`). */
const PRODUCER_ALIAS: Record<string, string> = {
  'meta-llama': 'meta',
...
/** Friendly labels for known producer slugs. */
...
/**
 * Ordered regex → slug rules for flat model ids (no `/` path).
 * Mirrors logo patterns in _PROVIDER_LOGO_RULES below.
 */
const SLUG_RULES: ReadonlyArray<readonly [RegExp, string]> = [
  [/qwen|alibaba/i, 'qwen'],
  [/google|gemini|gemma/i, 'google'],
  [/meta|llama(?![.\-_ ]?cpp)/i, 'meta'],
  [/mi[sx]tral|ministral/i, 'mistral'],
  [/deepseek/i, 'deepseek'],
  [/anthropic|claude/i, 'anthropic'],
  [/openai|gpt-|^o[13]-|chatgpt|dall-e/i, 'openai'],
  [/x-ai|xai|grok/i, 'xai'],
  [/cohere|command-r/i, 'cohere'],
  [/perplexity|sonar/i, 'perplexity'],
  [/nous|hermes/i, 'nous'],
  [/microsoft|phi-/i, 'microsoft'],
  [/zhipu|glm|chatglm/i, 'zhipu'],
  [/minimax/i, 'minimax'],
  [/kimi|moonshot/i, 'moonshot'],
  [/nvidia|nemotron/i, 'nvidia'],
];

/** Regex → inline SVG (trusted static strings only). */
const PROVIDER_LOGO_RULES: ReadonlyArray<readonly [RegExp, string]> = [
  [
...
function normalizeSlug(raw: string): string {
  ...
  return PRODUCER_ALIAS[lower] ?? lower;
}

function slugFromRegex(modelId: string): string | null {
  for (const [re, slug] of SLUG_RULES) {
    if (re.test(modelId)) return slug;
  }
  return null;
}
...
/** Friendly label for a producer slug. */
export function producerDisplayName(slug: string): string {
  const key = slug.trim().toLowerCase();
  if (PRODUCER_DISPLAY_NAMES[key]) return PRODUCER_DISPLAY_NAMES[key];
  if (!key || key === 'other') return 'Other';
  return key.charAt(0).toUpperCase() + key.slice(1).replace(/-/g, ' ');
}

/** Inline SVG logo for a model id, or null when no pattern matches. */
export function modelProducerLogoSvg(modelId: string): string | null {
  const trimmed = modelId.trim();
  if (!trimmed) return null;
  for (const [re, svg] of PROVIDER_LOGO_RULES) {
    if (re.test(trimmed)) return svg;
  }
  return null;
}
