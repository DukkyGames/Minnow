...
/** Remove file blocks and image placeholders; collapse extra blank lines. */
function stripAttachmentMarkers(content: string): string {
  const withoutFiles = content.replace(FILE_BLOCK_RE, '');
  const withoutImages = withoutFiles.replace(IMAGE_PLACEHOLDER_RE, '');
...
export function historyUserContentHasAttachments(content: string): boolean {
  ...
  return FILE_BLOCK_RE.test(content) || IMAGE_PLACEHOLDER_RE.test(content);
}
