...
|-------|----------|----------|
| **grill** | `/grilling` skill — ~20 `ask_question` cards, one at a time, recommended answer each | (chat only) |
...
- **Grill:** One `ask_question` card at a time; explore the codebase when a question is answerable from the repo.
