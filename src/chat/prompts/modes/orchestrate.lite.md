...
**Check-in tools:** `list_sub_agents`; `get_sub_agent_status({ run_id })` — session-scoped; includes `success` and `terminalReason` when terminal; if `success: false` or `terminalReason: "max_tool_turns"`, retry **`spawn_sub_agent`** or mark `failed`; never `complete`.
