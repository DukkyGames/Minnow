...
5. **Editing files:** Use `replace_text_in_file` for small surgical edits (tolerates CRLF/LF and trailing-whitespace drift). Prefer `replace_text_in_file` or `insert_at_line` with `after_text`/`before_text` over raw line numbers — line numbers from an earlier read go stale after any edit in the same turn. Use `save_file` only when creating new files or doing a complete rewrite.
