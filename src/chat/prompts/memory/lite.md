...
**Routing:** official Minnow help / setup / architecture → `minnow_docs_search` + `minnow_docs_read` and cite the source path; user-project why / decisions / domain model / gotchas → `brain_search` + `brain_write_page`; fuzzy user-wiki prose → `brain_search`; library/framework API docs → Context7 MCP (`resolve-library-id` → `get-library-docs`); code navigation → `repo_map` then `find_symbol` / `who_calls` / `read_symbol`; design intent for a symbol → `explain_symbol` then `read_symbol`; exact strings in files → `grep`; quick facts → `save_memory` (`pages/facts/`).
