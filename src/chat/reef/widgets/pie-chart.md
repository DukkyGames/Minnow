...
.rw-sliders label { display: flex; flex-direction: column; gap: 4px; font-size: 0.8125rem; font-weight: 400; color: var(--text-muted); min-width: 0; }
.rw-sliders input[type="range"] { width: 100%; accent-color: var(--accent); }
.rw-sliders .rw-val { font-family: var(--font-mono); font-size: 0.75rem; color: var(--text); }
.rw-chart {
...
const SLICE_COLORS = ['var(--accent)', 'var(--accent-dim)', 'var(--text-muted)', 'var(--accent)'];
