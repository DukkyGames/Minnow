...
.rw-card .rw-label { font-size: 0.75rem; font-weight: 400; color: var(--text-muted); margin-bottom: 6px; }
.rw-card .rw-value { font-size: 1.375rem; font-weight: 500; font-family: var(--font-mono); color: var(--text); line-height: 1.2; }
.rw-card .rw-delta {
  margin-top: 6px; font-size: 0.75rem; font-weight: 400; display: flex; align-items: center; gap: 4px;
}
.rw-delta.up { color: var(--accent); }
.rw-delta.down { color: var(--text-muted); }
.rw-delta .caret { font-size: 0.625rem; }
.rw-spark {
  margin-top: 8px; height: 36px; border: 0.5px dashed var(--border); border-radius: var(--radius-sm);
  background: var(--surface-elevated); display: flex; align-items: center; justify-content: center;
  font-size: 0.625rem; color: var(--text-muted); font-weight: 400;
}
</style>
