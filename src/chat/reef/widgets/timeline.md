...
.rw-dot {
  width: 12px; height: 12px; border-radius: 50%; border: 0.5px solid var(--mn-border-strong);
  background: var(--mn-surface-1); flex-shrink: 0; z-index: 1;
}
.rw-step[data-active] .rw-dot { background: var(--mn-accent); border-color: var(--mn-accent); }
.rw-line {
  flex: 1; width: 0.5px; min-height: 24px; background: var(--mn-border); margin-top: 4px;
}
.rw-step:last-child .rw-line { display: none; }
.rw-body { min-width: 0; padding-bottom: 2px; }
.rw-body h3 { margin: 0 0 4px; font-size: 0.875rem; font-weight: 500; color: var(--mn-fg); }
.rw-step[data-active] .rw-body h3 { color: var(--mn-accent); }
.rw-body p { margin: 0 0 6px; font-size: 0.8125rem; font-weight: 400; color: var(--mn-fg); line-height: 1.45; }
.rw-date { font-size: 0.75rem; color: var(--mn-fg-muted); font-weight: 400; }
</style>
