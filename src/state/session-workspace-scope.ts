...
/**
 * Legacy Expert Lab sessions stay out of the main sidebar; expert threads are listed
 * normally. App-scoped chats (Chat, Desktop, Email) belong to their own rails and are
 * never listed in the Code sidebar, even when their folder matches the Code workspace.
 */
export function isSidebarVisibleChat(chat: Chat): boolean {
  return chat.kind !== 'expert-lab' && chat.appScope == null;
}

/** True when the chat belongs to the MinnowOS Chat app. */
export function isChatAppChat(chat: Chat): boolean {
  return chat.appScope === 'chat';
}

/** True when the chat belongs to the MinnowOS desktop surface. */
export function isDesktopChat(chat: Chat): boolean {
...
/** True when the chat belongs to the MinnowOS Chat app. */
export function isAssistantChat(chat: Chat): boolean {
  return isChatAppChat(chat);
}

/**
 * Chats listed by the Chat app (newest first): its own threads, plus expert threads
 * bound to the chats sandbox.
 *
 * Experts have no app scope of their own, so they are still matched by path — safe
 * here because the chats sandbox is fixed server-side at `~/.minnow/chats` and,
 * unlike the desktop workspace, cannot be re-pointed by the user.
 */
...
/** Chat app rail rows: committed turns and/or an unsent draft. */
export function getSidebarListedAssistantChats(
  state: SessionState,
  chatsWorkspacePath = '',
): Chat[] {
  return getChatsForChatsWorkspace(state, chatsWorkspacePath).filter(chatHasListableContent);
}

/** Chat app rail rows (newest first). */
export function getAssistantChats(state: SessionState, chatsWorkspacePath = ''): Chat[] {
  return getSidebarListedAssistantChats(state, chatsWorkspacePath);
}

/**
 * Chats owned by the desktop surface (newest first).
 *
 * Deliberately not filtered by folder: desktop chats are one list, and each chat
 * carries the folder it was created in. Activating a chat moves the desktop
 * workspace to that folder, so the transcript, the file explorer, and the
 * selected workspace cannot drift apart.
 */
export function getDesktopChats(state: SessionState): Chat[] {
  return [...state.chats]
    .filter(isDesktopChat)
    .sort((a, b) => getChatLastMessageAt(b) - getChatLastMessageAt(a));
}

/** Desktop rail rows: committed turns and/or an unsent draft. */
export function getSidebarListedDesktopChats(state: SessionState): Chat[] {
...
/** True when a chat belongs to the Email app. */
export function isEmailAssistantChat(chat: Chat): boolean {
  return chat.appScope === 'email';
}

/** All Email-scoped chats in activity order, including the active empty chat. */
export function getEmailAssistantChats(state: SessionState): Chat[] {
  return [...state.chats]
    .filter(isEmailAssistantChat)
    .sort((a, b) => getChatLastMessageAt(b) - getChatLastMessageAt(a));
}

/** Email history rows with committed turns or a persisted composer draft. */
export function getListedEmailAssistantChats(state: SessionState): Chat[] {
  return getEmailAssistantChats(state).filter(chatHasListableContent);
}
...
export function resolveActiveEmailAssistantChatId(
  ...
  if (remembered) {
    const chat = state.chats.find(
      (candidate) => candidate.id === remembered && isEmailAssistantChat(candidate),
    );
    if (chat) return chat.id;
  }

  const scoped = getEmailAssistantChats(state);
...
/**
 * Pick the active Chat app thread: remembered id, else newest Chat app thread,
 * else create one bound to the chats workspace.
 */
export function resolveActiveAssistantChatId(
  ...
  if (remembered) {
    const chat = state.chats.find((c) => c.id === remembered && isChatAppChat(c));
    if (chat) return chat.id;
  }

  const scoped = getAssistantChats(state, key).filter(isChatAppChat);
  if (scoped.length) return scoped[0]!.id;
...
/**
 * Pick the active desktop thread: remembered id, else the newest desktop thread,
 * else create one in `desktopWorkspacePath`.
 *
 * Resolution is folder-independent — the remembered thread wins even when it was
 * created in another folder, and the caller re-points the desktop workspace at
 * that thread's folder.
 */
export function resolveActiveDesktopChatId(
  desktopWorkspacePath: string,
  state: SessionState,
  createScopedDesktopChat: (desktopWorkspacePath: string) => Chat,
): string {
  const remembered = getLastActiveChatIdForApp(state, DESKTOP_APP_ID);
  if (remembered) {
    const chat = state.chats.find((c) => c.id === remembered && isDesktopChat(c));
    if (chat) return chat.id;
  }

  const scoped = getDesktopChats(state);
  if (scoped.length) return scoped[0]!.id;
  ...
  return {
    ...
    workspacePath:
      typeof row.workspacePath === 'string'
        ...
        : '',
    ...(row.appScope === 'chat' || row.appScope === 'desktop' || row.appScope === 'email'
      ? { appScope: row.appScope }
      : row.appScope == null && row.modeId === 'desktop'
