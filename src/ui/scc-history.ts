...
export function createHistoryView(ctx: SccContext): SccView {
  ...
  async function selectCommit(sha: string): Promise<void> {
    ...
    renderDetail(sha, result.stdout ?? '', result.files ?? []);
  }

  function renderDetail(sha: string, stdout: string, nameStatus: GitFileEntry[]): void {
    const header = splitCommitOutput(stdout);
    const files = collectFiles(header.patch, nameStatus);
...
/** Split `git show` output into its header block and the unified patch. */
