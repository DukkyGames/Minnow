...
function openSkillOptions(skillId: string, label: string): void {
  ...
  slashStart = inputEl.value.lastIndexOf(`/${skillId}`);
  if (slashStart < 0) slashStart = 0;
...
function detectSlashContext(): void {
  ...
  if (pickerPhase === 'options' && pendingSkill) {
    const value = inputEl.value;
    const pos = inputEl.selectionStart;
    const before = value.slice(0, pos);
    const optionMatch = before.match(
      new RegExp(`(?:^|\\s)/${pendingSkill.id}\\s+([a-z0-9-]*)$`, 'i'),
    );
    if (optionMatch) {
      filterQuery = optionMatch[1] ?? '';
      activeIndex = 0;
      if (!open) {
        const leadingWs = optionMatch[0].startsWith(' ') ? 1 : 0;
        slashStart = (optionMatch.index ?? 0) + leadingWs;
        open = true;
        if (pickerEl) pickerEl.classList.remove('hidden');
        attachRepositionListeners();
      }
    ...
    const skillMatch = before.match(new RegExp(`(?:^|\\s)/${pendingSkill.id}\\s*$`, 'i'));
    if (skillMatch) {
      filterQuery = '';
      if (!open) {
        const leadingWs = skillMatch[0].startsWith(' ') ? 1 : 0;
        slashStart = (skillMatch.index ?? 0) + leadingWs;
        open = true;
        if (pickerEl) pickerEl.classList.remove('hidden');
        attachRepositionListeners();
      }
...
export function handleSkillPickerKeydown(e: KeyboardEvent): boolean {
  ...
  if (pickerPhase === 'options') {
    ...
    if (e.key === 'Enter' || e.key === 'Tab') {
      if (options.length > 0) {
        e.preventDefault();
        applySkillOption(options[activeIndex].id);
        return true;
      }
    }
    return false;
