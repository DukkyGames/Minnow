...
1. `live.mjs`: boot. If the request names or implies a file, route, or app inside a monorepo, infer the concrete path and run `node src/skills/impeccable/scripts/live.mjs --target <path>` instead; then run the rest of this live session from the returned `projectRoot`.
...
```bash
node src/skills/impeccable/scripts/live.mjs
```
...
```
LOOP:
  node src/skills/impeccable/scripts/live-poll.mjs   # default long timeout; no --timeout=
...
```
node src/skills/impeccable/scripts/live-poll.mjs --stream   # stays running; one JSON line per event
...
```bash
node src/skills/impeccable/scripts/live-status.mjs
node src/skills/impeccable/scripts/live-resume.mjs --id SESSION_ID
node src/skills/impeccable/scripts/live-complete.mjs --id SESSION_ID
```
...
```bash
node src/skills/impeccable/scripts/live-insert.mjs --id EVENT_ID --count EVENT_COUNT --position after \
...
```bash
node src/skills/impeccable/scripts/live-wrap.mjs --id EVENT_ID --count EVENT_COUNT --element-id "ELEMENT_ID" --classes "class1,class2" --tag "div" --text "TEXT_SNIPPET"
```
...
```bash
node src/skills/impeccable/scripts/live-poll.mjs --reply EVENT_ID done --file RELATIVE_PATH
```
...
```bash
node src/skills/impeccable/scripts/live-poll.mjs --reply EVENT_ID error "Short reason"
```
...
```bash
node src/skills/impeccable/scripts/live-poll.mjs --reply EVENT_ID steer_done ["Optional short note for a browser toast"]
```
...
```bash
node src/skills/impeccable/scripts/live-server.mjs stop
```
...
```bash
node src/skills/impeccable/scripts/detect-csp.mjs
```
