...
4. **React JSX:** bare import map specifiers (`react`, `react-dom/client`, `recharts`); `createRoot` from `react-dom/client`. In `style={{ }}`, quote tokens: `color: 'var(--mn-fg)'` — never `color: var(--mn-fg)` (invalid JS; widget will not mount).
