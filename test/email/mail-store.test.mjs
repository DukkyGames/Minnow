...
describe('inbox categories', () => {
  test('mixed-category thread rollup takes primary; a new primary promotes the thread', async () => {
    const threadId = '<mixed-cat@example.com>';
    await mergeMessagesIntoCache(
      ACCOUNT_ID,
      [
        message({
    ...
    let listed = await listCachedThreads(ACCOUNT_ID, { folder: 'INBOX' });
    assert.equal(listed.threads[0].category, 'other');
    ...
    listed = await listCachedThreads(ACCOUNT_ID, { folder: 'INBOX' });
    assert.equal(listed.threads[0].category, 'primary');

    const filtered = await listCachedThreads(ACCOUNT_ID, {
      folder: 'INBOX',
      category: 'primary',
    });
  ...
  test('migration is idempotent and category columns survive a second open', async () => {
    await mergeMessagesIntoCache(ACCOUNT_ID, [message()], 'INBOX', 1);
    const db = getMailDb(ACCOUNT_ID);
    const cols = db.prepare('PRAGMA table_info(messages)').all().map((row) => row.name);
    assert.ok(cols.includes('category'));
    assert.ok(cols.includes('category_source'));
    // Opening again must not throw (ensureColumn no-ops when present).
    closeMailDbs();
    const db2 = getMailDb(ACCOUNT_ID);
    assert.ok(db2.prepare('PRAGMA table_info(threads)').all().some((row) => row.name === 'category'));
  });

  test('backfill classifies empty category rows once', async () => {
    await mergeMessagesIntoCache(
      ACCOUNT_ID,
      [
        message({
          uid: '20',
          from: 'Notify <updates@linkedin.com>',
    ...
    // Simulate a pre-migration row with blank category.
    const db = getMailDb(ACCOUNT_ID);
    db.prepare("UPDATE messages SET category = '', category_source = ''").run();
    db.prepare("UPDATE threads SET category = ''").run();
    writeMeta(db, 'categoryBackfillAt', null);

    const first = await backfillCategoriesIfNeeded(ACCOUNT_ID);
    assert.equal(first.skipped, false);
    assert.ok(first.updated >= 1);

    const row = await getCachedMessage(ACCOUNT_ID, 'INBOX:20');
    assert.equal(row.category, 'social');

    const second = await backfillCategoriesIfNeeded(ACCOUNT_ID);
    assert.equal(second.skipped, true);
  });

  test('getCategoryUnreadCounts scopes to INBOX', async () => {
    await mergeMessagesIntoCache(
      ACCOUNT_ID,
      [
        message({
        ...
        message({
          ...
          threadId: '<sent-thread@example.com>',
          flags: { seen: false, flagged: false },
        }),
      ],
      'INBOX',
    ...
    const counts = await getCategoryUnreadCounts(ACCOUNT_ID, { folder: 'INBOX' });
    assert.equal(typeof counts.primary, 'number');
    assert.equal(typeof counts.social, 'number');
    assert.equal(typeof counts.other, 'number');
    assert.ok(counts.primary >= 1);
  });

  test('manual setThreadCategory pins source=user', async () => {
    await mergeMessagesIntoCache(
      ACCOUNT_ID,
      [message({ uid: '40', from: 'Promo <promo@brand.example.com>', subject: 'Deal' })],
      'INBOX',
      40,
    );
    const before = await getCachedMessage(ACCOUNT_ID, 'INBOX:40');
    await setThreadCategory(ACCOUNT_ID, before.threadId, 'social', { rememberSender: false });
    const after = await getCachedMessage(ACCOUNT_ID, 'INBOX:40');
    assert.equal(after.category, 'social');
    assert.equal(after.categorySource, 'user');
  });
});
