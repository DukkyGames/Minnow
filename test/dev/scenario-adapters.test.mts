/**
 * Scenario adapters for the fake-model host (MIN-513).
 *
 * Moved from `test/orchestrate/` in MIN-716. Settings board-testing adapters
 * emit `report_outcome` (V1 `board_report` is gone).
 */
...
describe('orchestrate scenario adapters', () => {
  test('produces valid ordered fake-model match/emit steps for the happy preset', () => {
    ...
    assert.equal(tool.function.name, 'report_outcome');
    assert.deepEqual(JSON.parse(tool.function.arguments), {
      outcome: 'pass',
      summary: 'Scenario adapter: report_outcome pass for W1-A',
      evidence: [],
      blockers: [],
      needs: [],
    });
  ...
  test('orders recoverable and terminal missing-report behavior before baseline steps', () => {
    ...
    assert.match(JSON.stringify(firstDelta(recoveryReport)), /report_outcome/);
