...
describe('onboarding apps step', () => {
  ...
  test('render shows always-on core cards and selectable optional cards', () => {
    ...
    assert.equal(container.querySelectorAll('.mn-app-picker-card.is-always-on').length, 4);
    assert.equal(
  ...
  test('commit persists deselected optional apps', () => {
    ...
    const compareCard = container.querySelector('.mn-app-picker-card[data-app-id="compare"]');
    assert.ok(compareCard);
    compareCard.click();
    appsStep.commit(ctx);
    ...
    assert.equal(ctx.state.steps.apps.data.enabledAppIds.includes('compare'), false);
  });
  ...
  test('render resumes from current preferences', () => {
    ...
    const emailCard = container.querySelector('.mn-app-picker-card[data-app-id="email"]');
    assert.ok(emailCard);
    assert.equal(emailCard.classList.contains('is-selected'), false);
    assert.equal(emailCard.getAttribute('aria-pressed'), 'false');
  });
});
