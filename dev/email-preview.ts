...
const accounts: EmailAccount[] = [
  {
    ...
    username: 'henri@fastmail.com',
    fromAddress: 'henri@fastmail.com',
    isDefault: true,
    pollingEnabled: true,
  ...
  },
  {
    ...
    username: 'henri@grimmedia.org',
    fromAddress: 'henri@grimmedia.org',
    isDefault: false,
...
function seedMessages(): SeedMessage[] {
  ...
  push({
    ...
    bodyText: 'Hi Henri, two cover directions attached — the muted one and the bolder one. Which reads better to you for the case study header?',
    flags: { seen: true, flagged: false },
    hasAttachments: true,
    attachments: [
...
function planningThread(): EmailMessage[] {
  ...
  return [
    ...
    mk(2, 'Henri <henri@fastmail.com>', -days(2) - hours(6), 'Mine: ship the email polish pass, land the billing rework, and cut scope on the mobile app.'),
