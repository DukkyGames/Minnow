...
**Code map injection** (optional per send): ranked signature map from the Brain **code index** injects into the `code-map` prompt part when resolved on ([`src/brain/code-injection-config.ts`](../src/brain/code-injection-config.ts), [`src/chat/prompts/compose-context.ts`](../src/chat/prompts/compose-context.ts)). Global default `features.codeMapInjectionDefault` (boolean, default **false**); per-chat tri-state `chat.codeMapInjection` (`inherit` \| `on` \| `off`, same semantics as thinking). Composer **code overview** toggle mirrors the thinking control ([`src/ui/composer-code-map.ts`](../src/ui/composer-code-map.ts)). Token budget uses Settings → Brain → Code **repo map token budget**; fetch goes through `POST /api/brain/code/repo-map` with `repo` (worktree-aware workspace key) and `ensureIndexed` (no `focus` — injection is the full PageRank-ranked map, not a substring filter on the user message; Brain Code **filter map** and `repo_map` tool still accept optional `focus`). Map body is wrapped `source="code-map"` ([`src/brain/code-map-injection.ts`](../src/brain/code-map-injection.ts)). Context usage ring + breakdown ([`src/ui/context-usage-ring.ts`](../src/ui/context-usage-ring.ts), [`src/chat/context-usage.ts`](../src/chat/context-usage.ts)): **Code map** breakdown row when injected (system row excludes map tokens; map tokens count toward the single ring **Used** total); **Code map (loading)** when injection is on but the map fetch is still empty.

**Injection transcript rows** (UI-only): after each user send, when Brain retrieve or code-map injection produced a **non-empty** raw block for that compose pass, the chat transcript gets left-aligned **tool-call-style** disclosure rows (glyph · action · line-count outcome · chevron), same chrome as [`tool-call-msg`](../src/styles/messages.css) ([`src/styles/context-notice.css`](../src/styles/context-notice.css), [`src/ui/messages.ts`](../src/ui/messages.ts)). Expand shows the retrieved payload (`memoryBlock` / `codeMapBlock`), not the full `memory` / `code-map` markdown templates. Rows use `role: 'injection'` ([`src/chat/context/injection-notice.ts`](../src/chat/context/injection-notice.ts)); they are omitted from API history, token estimate, synthesis, compress, and orchestrate failure classification (same rule as `role: 'context'`). Wired from [`resolveOutboundSystemMessages`](src/chat/prompts/compose-context.ts) in the tool loop and plain streaming chat path.
