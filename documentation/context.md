...
**Server Session Engine (MIN-354 Phase 1 / MIN-359):** Behind `MINNOW_SERVER_ENGINE` (env, default **off**). When on: composer main-chat sends dispatch `POST /api/session/commands` (`send_message` / `stop_generation` / `steer_message` / `enqueue_message`) instead of renderer `runChatTurn`; the engine owns an in-memory SessionState, runs a DOM-less tool loop ([`src/session-engine/main-chat-loop.ts`](../src/session-engine/main-chat-loop.ts), loaded via tsx), creates `/api/generations` streams, and publishes committed history via Phase 0 SSE. Client helpers: [`src/state/session-commands.ts`](../src/state/session-commands.ts), flag probe [`src/state/server-engine-flag.ts`](../src/state/server-engine-flag.ts) (`window.__MINNOW_SERVER_ENGINE__` injected with the session token), token mirror [`src/chat/engine-stream-mirror.ts`](../src/chat/engine-stream-mirror.ts). Board scheduler stays renderer + Phase 0 lease. Engine core: [`server/session/engine.js`](../server/session/engine.js) + [`commands.js`](../server/session/commands.js) — mutates are serialized; soft mid-turn flushes cannot overwrite a later hard commit; idle client PUT/PATCH adopts into engine memory (skipped while a turn is active). Flag off ⇒ existing `loop.ts` path unchanged.
...
Browser / Electron (same origin, default :9473)
  ...
  ├─ POST /api/session/commands      → typed commands → 202 { rev } (Phase 1, flag on)
