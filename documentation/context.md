...
| `GET/POST /api/brain/code/repo-map` | Token-budgeted signature map (`focus?`, `tokenBudget?`) |
...
**Task chat:** `startTask` creates or reuses a **Build-mode** chat (planner’s provider/model, default **Builder** work agent), sets `Chat.boardTaskId` to the board task id, seeds a user message (plan path + task id + build/test specs), and runs `runChatTurn` in the background without switching the active chat. **Testing (2026-06):** successful builds land in **Testing**; `startTaskTesting` runs the **Tester** work agent ([`src/chat/prompts/work-agents/tester/`](../src/chat/prompts/work-agents/tester/)) in a linked `testChatId` chat (headless per-task: typecheck/lint/unit/build + static review; no browser). Verdict via **`board_report_test_result`** → `finalizeTaskTestingOnStreamEnd` (`pass` → `complete`; `fail`/missing → fresh Builder retry, ≤3 → `blocked`). When every task is `complete`, **`startFinalIntegrationTest`** runs Tester with browser smoke (single shared final pass); board completion UI waits for `finalTest.status === 'passed'`. Manual mode: **Run tests** / **Run final integration test** buttons; manual drag Testing → Complete still allowed. **`board_get_state` / `board_update_task`** resolve the folder’s planner chat from `ExecuteToolContext.chatId` (main loop, sub-agent parent, or linked task chat via `boardGroupId`); **`board_init`** still requires an Orchestrate-mode planner chat. **Stop** calls `stopGeneration` on build and test chats and keeps transcripts. Default **`maxConcurrentTasks`** = 3 on the board (queued wave starts drain on `notifyChatStreamEnded`). **Task errors** (`.board-task-card__error`, from failed sub-agent runs or `board_update_task`) are cleared when a task is **Reopen**ed (`moveTaskStatus` → `planned`) or **Start**ed again. **Board return:** task-card chat rows call `switchChat`; reopening the board (sidebar folder header or board toggle) calls `openBoardGroup`, which focuses the planner chat when a member task chat was active. While `ChatGroup.viewMode === 'board'`, `isStreamDomVisible` / `appendBubble` suppress live DOM for **all** active chats (including Build-mode task streams), except the transient orchestrator init split.

**Tests:** `test/chat/concurrent-streams.test.mts`, `test/state/chat-groups.test.mts`, `test/ui/sidebar-order.test.mts`, `test/orchestrate/plan-complete.test.mts`, `test/orchestrate/board-store.test.mts`, `test/orchestrate/delegate-tasks.test.mts`, `test/orchestrate/task-stream-end.test.mts`, `test/orchestrate/task-testing.test.mts`, `test/chat/orchestrate/plan-from-history.test.mts`, `test/chat/orchestrate/plan-preview.test.mts`, `test/chat/orchestrate/task-activity.test.mts`, `test/chat/orchestrate/task-chats.test.mts`, `test/ui/orchestrate-board-header-status.test.mjs`, `test/ui/orchestrate-board-live-update.test.mjs` (wave collapse, task card activity/chats), `test/chat/modes/composer-modes.test.mts`, `test/ui/orchestrate-hub.test.mts`, `test/ui/orchestrate-plan-screen.test.mts`.
...
**Prompt token estimate (Feature 25 / F4):** While settings is open, `#settingsPromptTokenEstimate` in `.settings-page-header` (right-aligned via `margin-left: auto`) shows **~N tokens (estimate)** for the next main-chat send (active session). **Prompting** adds `#settingsPromptTokenBreakdown` (System · History · Tools · Rules). Heuristic: `chars ÷ 4` (`estimateTokensFromText` in `src/chat/prompts/token-estimate-core.ts`). `resolveOutboundPromptEstimate()` in `src/chat/prompts/token-estimate.ts` mirrors send via `resolveOutboundSystemMessages()`, API-shaped history (`historyToApiMessagesForEstimate` — excludes assistant `thinking`), mode-filtered tool JSON (work-agent allowlist + UI Designer filter), and **per-agent context budget trimming** (`applyContextBudget()` when the active work agent has `maxInputTokens`). UI: `src/ui/settings-prompt-estimate.ts`. Refreshes on settings open, section change, and profile/part toggles (300 ms debounce). Not provider `usage.prompt_tokens`. Verification: [`documentation/plans/verification/feature-25.md`](plans/verification/feature-25.md).
...
**Context window usage (MIN-13 / BUG-019 / MIN-159):** In-chat **context fill** indicator distinct from the bottom metrics strip (tok/s, TTFT). `#contextUsageRing` in `#composerControls` (after the thinking toggle): compact 24px SVG ring (no button chrome); light grey track, ink `--accent` stroke fill for used share; warning fill at ≥85% (`--warning`). Hover tooltip: model name, limit, used/remaining (approx.). Click opens `#contextUsageBreakdown` popover with per-section token rows and bars (System, Rules, Tools, History, optional **In progress (estimate)**, Composer/Attachments). Data: `getContextBudget()` / `assembleContextBudget()` in [`src/chat/context-usage.ts`](../src/chat/context-usage.ts) merges `resolveOutboundPromptEstimate()` (API-shaped history without assistant `thinking`, context-budget-trimmed when agent cap set) + pending `#msgInput` + `getPendingAttachments()` (`estimateAttachmentTokens` caps images at `ESTIMATE_IMAGE_URL_TOKENS` = 256, not full base64) + in-flight overlay from [`src/chat/context-in-flight.ts`](../src/chat/context-in-flight.ts) (**pending tool-call JSON only** — streaming completion prose and live reasoning are output, not prompt). **Context limit** uses the effective window, not catalog max: `resolveContextLimit()` prefers live `loaded_context_length` / `max_context_length` from the in-memory model catalog (`getModelRowForSelectOrCanonicalId` / `fetchModels()`), then falls back to `chat.modelInfo.context_length` from the last completion when the cache is unavailable (MIN-183), then a **known-model lookup** for `openai-v1` providers when upstream `/v1/models` omits context metadata (MIN-199; table in [`src/lib/known-context-windows.ts`](../src/lib/known-context-windows.ts); server proxy preserves upstream context fields via `normalizeOpenAiModelRow()` in [`server/providers/paths.js`](../server/providers/paths.js)). Row precedence matches `contextLengthFromModelRow()` in [`src/lib/context-length.ts`](../src/lib/context-length.ts). `resolveModelInfo()` applies the same live-first rule for the stats strip `#iCtx` field. Shows **last turn API** `prompt_tokens` when `chat.lastStats` has them; section sizes stay heuristic. UI: [`src/ui/context-usage-ring.ts`](../src/ui/context-usage-ring.ts), [`src/ui/context-usage-breakdown.ts`](../src/ui/context-usage-breakdown.ts), [`src/styles/context-usage.css`](../src/styles/context-usage.css). Refreshes on history paint (`renderChatFromHistory` / `renderStatsForChat`), model change, composer input, attachments, tool permission changes, and **during active turns** via debounced `scheduleContextUsageRefresh()` from [`src/tools/loop.ts`](../src/tools/loop.ts) (SSE prose/reasoning deltas, each `chat.history.push`, turn `finally`). `updateStrip()` alone does not refresh the ring. Tests: [`test/chat/context-usage.test.mts`](../test/chat/context-usage.test.mts), [`test/chat/outbound-prompt-estimate.test.mts`](../test/chat/outbound-prompt-estimate.test.mts).

**Models hub (Feature 02):** Settings → **Models** (`#/settings/model-routing`, nav label; hash id unchanged) is the consolidated per-role surface: **Main chat** (top-bar provider/model + global sampler + per-chat thinking), work agents, sub-agent types, background jobs (UI Designer, titles), and Reef widget LLM on the **active chat**. Each row has provider/model selects, **Effective** readout, an **Advanced (sampler · thinking)** disclosure for roles that support it, and a per-row **Fallback chain** disclosure (ordered provider/model candidates saved under `config.json` → `fallbackChains.roles[<routing-row-id>]`). **Model fallback (Odysseus port #04 / MIN-128):** top of the Models section — global enable toggle, dead-host cooldown, **Global fallback chain** (`fallbackChains.roles._global`, last resort when a role has no chain or its candidates fail), and diagnostics (`GET /api/system/host-health`); default **off**. Legacy coarse role keys (`default`, `utility`) still resolve via aliases (`main-chat`, `chat-titles`). Catalog: `src/settings/model-routing-catalog.ts`; UI: `src/ui/settings-model-routing.ts`; config client: `src/config/fallback-chains-meta.ts`; shared selects: `src/ui/settings-model-binding.ts` (`inline` layout). Entity sections link here instead of duplicating bindings.
...
**Thinking mode toggles:** Settings → **Thinking** (`#/settings/thinking`) sets global default **`on`** (`config.json` → `thinking.defaultMode` via `src/config/thinking-meta.ts`). Per-role tri-state overrides (`inherit` | `on` | `off`) live in **Models** → Advanced. Composer `#composerThinkingControl` is a single brain icon toggle (`src/ui/composer-thinking.ts`, `/icons/thinking-brain.png`) inline after mode segments: **on** / **off** appearance (muted when off), default **inherit** (shows resolved stack; tooltip), click cycles override (`inherit` → opposite of resolved → toggle explicit → back to inherit). Writes `Chat.thinkingMode`. Resolution: `src/agents/resolve-thinking.ts`; request fields: `src/agents/thinking-to-body.ts` (`openai-v1` sends `thinking: { type: 'enabled' | 'disabled' }` for Kimi/DeepSeek compat — no `enable_thinking` or `reasoning_effort`; `lm-studio-v0` uses `enable_thinking` + `reasoning_effort`). Plan: [`documentation/plans/Build out/feature-thinking-mode-toggles.md`](plans/Build%20out/feature-thinking-mode-toggles.md).
...
| Foreground detect | [`src/ui/chat-mount.ts`](../src/ui/chat-mount.ts) — `isChatAppForeground()` (desktop chat, foreground `chat` instance, or legacy `#chatView.is-open` when no other OS app is foreground) drives composer, mount, stream DOM, and tool `workspaceRoot`. **MIN-172:** returns false when Code is foreground even if desktop chat state is still `chatActive` (preserved for return navigation) so the file tree uses the project workspace, not `~/.minnow/chats`. `getActiveChatMountElement()` applies the same Code-first rule. |
...
| Content-json tool merge | `src/providers/constrained-tool-content.ts` — `toolArgumentsRichnessScore` + `mergeContentJsonToolCalls` prefer fuller assistant-content JSON over partial SSE `arguments` (e.g. `board_init` with only `plan_path`); wired in `src/tools/loop.ts` `streamCompletionTurn` |
...
Full-page **Models** hub at `#/app/models/<section>` (`src/ui/models-page.ts`, `src/styles/models-page.css`). Sections: **What fits** (hardware-ranked catalog + local cache badges; recommend rows place fit badge, **Download** / **Serve**, and on-disk dot inline in `models-recommend-row__trailing`; download progress renders below the meta line; meta shows **file size** (`size_gb`, weights-only estimate from `estimateFileSizeGb()` in `src/models/quant.ts`) and **RAM to run** (`required_gb`, includes KV cache); **filter toolbar** in `renderFilters()` (`src/ui/models/recommend-panel.ts`) is a single-row instrument bar: search (borderless, flex-grow) | use-case / quant / sort selects | context label + mono value + range | **Fit only** checkbox (nested label without duplicate `htmlFor`; when unchecked, `rankModels()` in `src/models/fit.ts` reserves ~25% of the result limit for `too_tight` rows so they are not pushed out by score sort), groups separated by hairline dividers in `models-page.css`; **GPU budget** toggle in `src/ui/models/recommend-panel.ts` maps RAM-only vs GPU pool through `hardwareForGpuBudget()` in `src/models/fit.ts` — RAM sets `hasGpu: false`, GPU sets `gpu_only: true` on the selected `gpuGroups` pool; **use-case filter** dropdown values are `inferUseCase()` keys — `general`, `coding`, `reasoning`, etc. — not raw catalog `use_case` strings), **Installed** (downloads + serve), **Settings** (HF token + extra model dirs), plus reparented provider/routing/sampler/thinking/usage settings. Server: [`server/models/`](../server/models/) via [`createModelsMiddleware()`](../server/models/routes.js).
...
| Concern | Location |
|---------|----------|
| Backend | `server/generations/store.js`, `upstream.js`, `routes.js`, `fallback.js`, `host-cooldown.js`; wired in `server.js` |
...
**Failed-turn history (MIN-184):** When a main-chat generation fails after partial tool-loop output, `runChatTurn` rolls `chat.history` back to the fork user row (`rollbackFailedTurnHistory` in [`src/chat/history.ts`](../src/chat/history.ts)), re-renders the transcript, and does not persist partial run output. `buildApiMessages` serializes a sanitized copy via `copyHistoryForOutboundApi` (`normalizeHistoryTail` on a clone). New sends call `clearPostToolTailBeforeSend` for legacy poisoned sessions; error bubbles can offer **Clear last failed turn** (`setAssistantErrorBubbleWithRecovery` → `resendFromIndex`). Tests: [`test/chat/history.test.mts`](../test/chat/history.test.mts).
...
- **Reasoning / “thinking”** (LM Studio **App Settings → Developer**: separate `reasoning_content` and/or `choices.delta.reasoning` for compatible models such as DeepSeek R1 / gpt-oss):
  ...
  - **Stream routers:** [`InlineContentThinkingRouter`](../src/api/inline-thinking.ts) + [`HarmonyChannelRouter`](../src/api/inline-thinking.ts) wired in [`loop.ts`](../src/tools/loop.ts), [`chat.ts`](../src/api/chat.ts), and [`sub-agent-runner.ts`](../src/agents/sub-agent-runner.ts) so live `delta.content` routes to thought bubbles instead of the prose bubble.
