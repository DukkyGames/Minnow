...
**Legacy `#chatView`:** DOM retained for non-OS mode and notification deep-links; desktop chat (`#desktopChatCol`) is primary when the OS shell is enabled. Full removal deferred — see [`desktop-window-shell.md`](plans/desktop-window-shell.md) known items.

**Tests:** `test/os/window-manager.test.mts`, `window-apps.test.mts`, `desktop-chat-state.test.mts`, `desktop-research-state.test.mts`, `desktop-experts-state.test.mts`, `scheduler-app.test.mts`, `calendar-app.test.mts`, `router.test.mts`.
