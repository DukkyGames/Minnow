...
**Migration:** [`server/brain/migrate.js`](../server/brain/migrate.js) — one-time `memory/` → `pages/facts/<slug>.md` (preserves ids). **Memory back-compat:** [`server/memory/{routes,store,middleware}.js`](../server/memory/routes.js) delegate to brain. **MIN-B4 (shipped):** wiki tools registered in [`src/tools/definitions.ts`](../src/tools/definitions.ts) + [`SERVER_TOOL_HANDLERS`](../server/runtime/tools-middleware.js); prompt partials [`memory/full.md`](../src/chat/prompts/memory/full.md) / [`lite.md`](../src/chat/prompts/memory/lite.md) describe routing schema. **Out of scope (MIN-B4):** code tools (MIN-B7), `explain_symbol` (MIN-B9), SPA Brain app (MIN-B5). **Tests:** `npm run test:brain`, `npm run test:memory`. Plans: [`MIN-B2`](plans/brain/MIN-B2-wiki-store.md), [`MIN-B3`](plans/brain/MIN-B3-brain-routes-retrieve-adapter.md), [`MIN-B4`](plans/brain/MIN-B4-wiki-tools-prompt.md).
