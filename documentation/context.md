...
**Expert Lab:** Three-step flow (pick tile grid → brief → run timeline). Runs use `runChatTurn` with `composedSystemPromptOverride` = expert `fullBody`; stream hooks in `src/ui/expert-lab-stream.ts` (called from `src/tools/loop.ts`). Phases: Understanding, Clarifying (`ask_question` + `#questionHost` reparent), Working (token count + spinner), Output (live markdown via debounced `scheduleAssistantBubbleRender` on `#expertLabOutputBubble` from `onPartialText` in [`expert-lab-page.ts`](../src/ui/expert-lab-page.ts); final pass `setAssistantBubbleContent` on `onRunEnd`). Sidebar filters `kind: 'expert-lab'`. Meta: optional `icon` + `accent` (`sage` \| `amber` \| `cyan` \| `coral` \| `violet` \| `rose`) in expert front matter.
