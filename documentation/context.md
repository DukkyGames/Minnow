...
**Also useful:** [`guides/configuration.md`](guides/configuration.md) (storage layout), [`guides/release-e2e-testing.md`](guides/release-e2e-testing.md) (manual pre-release checklist), [`DESIGN.md`](../DESIGN.md) (visual tokens), [`AGENTS.md`](../AGENTS.md) (agent quick reference), [`plans/`](plans/) (in-flight feature plans).
...
**Education Mode** is a global overlay (not a composer mode): when enabled via **Settings → General**, every mode becomes a read/run/review tutor — write tools are stripped from exposure, dispatch is guarded client- and server-side ([`education-overlay.ts`](../src/chat/modes/education-overlay.ts), [`education-guard.ts`](../src/chat/modes/education-guard.ts), [`server/tools/education-guard.js`](../server/tools/education-guard.js)), sub-agents inherit the overlay, and teaching prompts append from [`education.md`](../src/chat/prompts/tool-usage/education.md) / lite variant. Students still edit via the Code viewer and file tree (`toolCaller: user` on those tool calls). The tutor prompt states the environment — Minnow *is* the student's editor, workspace at `{{cwd}}` — so it never asks which IDE they use, and rung 2 of the hint ladder is an `open_in_editor` call that highlights the lines rather than a description of where to look. Config: `config.json` → `education` (`enabled`, `level`: beginner \| intermediate \| advanced); sync cache [`education-meta.ts`](../src/config/education-meta.ts). Composer badge: [`composer-education-badge.ts`](../src/ui/composer-education-badge.ts). `execute_command` stays available (tests/repro) with a best-effort shell denylist for file-mutating commands — not a sandbox.
...
| **Editor view** | `open_in_editor` (opens a file in the Code viewer, selects a line range) | Browser ([`open-editor-tool.ts`](../src/tools/open-editor-tool.ts)) |
...
**Testing guide:** [guides/orchestrate-board-testing.md](guides/orchestrate-board-testing.md) — `npm run test:board`, fake model, `seed:test-board`, `check:board-log`, harness layout. **Manual workflow GUI:** Settings → **Advanced → Board testing** ([`src/ui/settings-board-testing.ts`](../src/ui/settings-board-testing.ts)) — scenario catalog + runner (`GET/POST /api/orchestrate/board-testing/runs/*`), in-process fake model (`POST /api/orchestrate/board-testing/fake-model/*`), seed test board (including `afk` mode), validate board log JSONL. API: [`server/orchestrate/board-testing/`](../server/orchestrate/board-testing/). Shared seed presets: [`src/dev/test-board-seed.ts`](../src/dev/test-board-seed.ts). Unified scenario catalog: [`src/dev/orchestrate-scenarios/`](../src/dev/orchestrate-scenarios/).
...
**LSP:** Bundled TS/JS + on-demand language bundles; config `~/.minnow/lsp.json`, defaults `src/lsp/defaults.json`. APIs: `/api/lsp/*`. TypeScript 7 no longer ships `tsserver.js`; `tsserver-fallback` (npm alias to TS 5.8) supplies the bundled fallback path for `typescript-language-server`. Agent-scoped `get_lsp_diagnostics` resolves paths and spawns language servers against the active tool `workspaceRoot` (board/chat worktree override via `getEffectiveWorkspaceRoot()`), with per-root agent connections so `initialize` `rootUri` matches the isolated checkout.
