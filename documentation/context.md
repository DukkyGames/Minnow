...
**Scheduler app** (`#/app/scheduler`, [`src/ui/scheduler-page.ts`](../src/ui/scheduler-page.ts)): full-page job CRUD in `#schedulerPanelMount` via [`scheduler-panel.ts`](../src/ui/scheduler/scheduler-panel.ts) — uptime notice, mono job/enabled stats, stacked bordered job rows (humanized schedule via [`schedule-display.ts`](../src/scheduler/schedule-display.ts), next run, workspace + model labels, prompt preview, inline actions), side editor with friendly schedule builder ([`schedule-field.ts`](../src/ui/scheduler/schedule-field.ts): interval shorthand or cron presets + live next-run preview), optional per-job model (multi-provider picker — same combobox UI as the menubar chip; empty = menubar default at run time), and optional per-job workspace (folder picker; unset → `~/.minnow/scheduler-workspace`), teaching empty state, and run-history table. Default workspace: [`server/scheduler-workspace/paths.js`](../server/scheduler-workspace/paths.js) (`GET /api/scheduler/workspace`). Run-time model resolution: [`server/scheduler/resolve-job-model.js`](../server/scheduler/resolve-job-model.js) → explicit job fields or active chat from `~/.minnow/sessions/state.json` (`providerId` + `modelId` on chats preserved by [`validateSessionState`](../server/config/validators.js)). Styles: [`src/styles/scheduler-page.css`](../src/styles/scheduler-page.css). Persistence: `~/.minnow/scheduler.json` (MIN-120). Manual and scheduled runs spawn [`bin/minnow.mjs`](../bin/minnow.mjs) → tsx + [`test/test-loader.mjs`](../test/test-loader.mjs); on Windows the `--import` hook must be a `file://` URL (`pathToFileURL` in `bin/minnow.mjs`). Job subprocesses use the Vite-resolved origin from [`server/scheduler/server-base-url.js`](../server/scheduler/server-base-url.js) (`http://localhost:PORT`, not `127.0.0.1` — Vite on Windows often binds IPv6 loopback only).
