...
**Calendar app (MIN-129, shipped):** MinnowOS **Calendar** app (`#/app/calendar`) — local-first SQLite store at `~/.minnow/calendar/calendar.db` ([`server/calendar/store.js`](../server/calendar/store.js)), month/week grid UI ([`src/ui/calendar-page.ts`](../src/ui/calendar-page.ts), [`src/ui/calendar/calendar-panel.ts`](../src/ui/calendar/calendar-panel.ts) — cell-anchored **day popover** for `+N more` and date-label clicks; centered **event popup** (create/edit overlay, backdrop dismiss + Escape) for **New event** and chip clicks; view-specific inline limits via [`src/calendar/prefs.ts`](../src/calendar/prefs.ts) (`minnow.calendar.prefs`); **Settings** toolbar modal [`src/ui/calendar/calendar-settings.ts`](../src/ui/calendar/calendar-settings.ts) for display prefs, local calendar CRUD, and full reset; sidebar **CalDAV** section to add/remove accounts and per-account sync), `.ics` import/export ([`server/calendar/ics.js`](../server/calendar/ics.js)), RRULE expansion ([`server/calendar/recurrence.js`](../server/calendar/recurrence.js)), encrypted CalDAV accounts (#12 secret-box at `~/.minnow/calendar/secrets/`), bidirectional sync via `tsdav` ([`server/calendar/caldav.js`](../server/calendar/caldav.js)), SSRF validation ([`server/calendar/caldav-ssrf.js`](../server/calendar/caldav-ssrf.js)), REST `/api/calendar/*` ([`server/calendar/middleware.js`](../server/calendar/middleware.js) — includes `PUT`/`DELETE /api/calendar/calendars/:id` and `POST /api/calendar/reset` with `{ confirmed: true }`), agent tool `manage_calendar` ([`server/calendar/tool-handler.js`](../server/calendar/tool-handler.js) — list supports `limit`/`offset` with visible truncation), and upcoming-event reminders that enqueue into the scheduler notification queue ([`server/calendar/reminders.js`](../server/calendar/reminders.js) → `GET /api/calendar/upcoming`). Client API: [`src/calendar/client.ts`](../src/calendar/client.ts). Styles: [`src/styles/calendar.css`](../src/styles/calendar.css). Plan: [`documentation/plans/calendar-fixes-plan.md`](plans/calendar-fixes-plan.md). Tests: `npm run test:calendar`.

**Email app (MIN-126, shipped):** MinnowOS **Email** app (`#/app/email`) — encrypted IMAP account setup ([`server/email/accounts.js`](../server/email/accounts.js)), read-only folder sync + local cache ([`server/email/imap.js`](../server/email/imap.js), [`server/email/cache.js`](../server/email/cache.js)), thread grouping ([`server/email/threads.js`](../server/email/threads.js)), AI triage with #13 untrusted wrapping ([`server/email/triage.js`](../server/email/triage.js)), opt-in polling ([`server/email/poller.js`](../server/email/poller.js)), draft + explicit-confirm SMTP send ([`server/email/smtp.js`](../server/email/smtp.js)), agent tools `list_mail` / `draft_reply` ([`server/email/tool-handler.js`](../server/email/tool-handler.js)). UI: [`src/ui/email-page.ts`](../src/ui/email-page.ts), [`src/ui/email/email-panel.ts`](../src/ui/email/email-panel.ts). Client API: [`src/email/client.ts`](../src/email/client.ts). Styles: [`src/styles/email.css`](../src/styles/email.css). Legacy `#/email` → `#/app/email`. **Limitations:** IMAP/SMTP basic auth only in v1 — many Outlook/Microsoft 365 tenants require Graph/OAuth instead. Tests: `npm run test:email`.
