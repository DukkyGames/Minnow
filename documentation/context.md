...
**Code map injection** (optional per send): ranked signature map from the Brain **code index** injects into the `code-map` prompt part when resolved on ([`src/brain/code-injection-config.ts`](../src/brain/code-injection-config.ts), [`src/chat/prompts/compose-context.ts`](../src/chat/prompts/compose-context.ts)). Global default `features.codeMapInjectionDefault` (boolean, default **false**); per-chat tri-state `chat.codeMapInjection` (`inherit` \| `on` \| `off`, same semantics as thinking). Composer **code overview** toggle mirrors the thinking control ([`src/ui/composer-code-map.ts`](../src/ui/composer-code-map.ts)). Token budget uses Settings → Brain → Code **repo map token budget**; fetch goes through `POST /api/brain/code/repo-map` with `repo` (worktree-aware workspace key), `ensureIndexed`, and `focus` from the outgoing user message. Map body is wrapped `source="code-map"` ([`src/brain/code-map-injection.ts`](../src/brain/code-map-injection.ts)). Context usage ring + breakdown ([`src/ui/context-usage-ring.ts`](../src/ui/context-usage-ring.ts), [`src/chat/context-usage.ts`](../src/chat/context-usage.ts)): **Code map** breakdown row when injected (system row excludes map tokens; map tokens count toward the single ring **Used** total); **Code map (loading)** when injection is on but the map fetch is still empty.
