...
Orchestrate mode is **parse-only** for the LLM: read the plan, call **`board_init`** once (per-task `build` / `test`), then stop. Execution is **manual** on the board and in linked **task chats** (no automated supervisor yet; auto-pilot is planned in [MIN-140](plans/Orchestrator%20Plan.md) Phase 3–4).
...
| Controller | `src/agents/controller/{controller,scheduler,registry,watchdog}.ts` — spawn, cancel, queue, `restartSubAgent`, `cancelAllForParentTurn`, parent-chat/turn indexes, list/status helpers; `deriveSubAgentTerminalReason` + `terminalReason` on `get_sub_agent_status` / aggregate JSON. **`src/agents/orchestrator.ts`** is a thin re-export shim (naming maps to the prompt's "controller"). **MIN-140** Phase 3 adds heartbeat/watchdog supervision in `watchdog.ts`. |
| Repetition detector | `src/agents/self-healing/detector.ts` — pure `detectRepetition` (reused by future watchdog) |
