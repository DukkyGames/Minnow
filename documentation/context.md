...
**Migration:** [`server/brain/migrate.js`](../server/brain/migrate.js) — one-time `memory/` → `pages/facts/<slug>.md` (preserves ids). **Memory back-compat:** [`server/memory/{routes,store,middleware}.js`](../server/memory/routes.js) delegate to brain. **MIN-B4 (shipped):** wiki tools registered in [`src/tools/definitions.ts`](../src/tools/definitions.ts) + [`SERVER_TOOL_HANDLERS`](../server/runtime/tools-middleware.js); prompt partials [`memory/full.md`](../src/chat/prompts/memory/full.md) / [`lite.md`](../src/chat/prompts/memory/lite.md) describe routing schema. **MIN-B5 (shipped):** MinnowOS **Brain** app (`#/app/brain/<section>`) — window-mode wiki UI with sections **Wiki, Edit, Log, Schema, Proposals, Ingest, Lint, Settings** ([`src/ui/brain-page.ts`](../src/ui/brain-page.ts), [`src/brain/client.ts`](../src/brain/client.ts), lazy [`src/styles/brain-page.css`](../src/styles/brain-page.css); FOUC guard in [`global.css`](../src/styles/global.css)). Wiki viewer rewrites `[[path]]` wikilinks via [`wikilink-markdown.ts`](../src/ui/brain/wikilink-markdown.ts); Proposals reuses [`memory-proposals-panel.ts`](../src/ui/memory-proposals-panel.ts); Settings mirrors synthesis + embeddings controls (`config.synthesis` / `config.memory.embeddings`). **MIN-B7 (shipped):** deterministic **code index** under `~/.minnow/brain/code/<workspace-key>.db` — LSP `documentSymbol` + `callHierarchy` → SQLite symbols/edges, personalized PageRank, token-budgeted repo map, read-only tools (`repo_map`, `find_symbol`, `who_calls`, `read_symbol`). Modules: [`server/brain/code/{schema,indexer,rank,repo-map,query,routes,config}.js`](../server/brain/code/schema.js), handlers [`server/tools/code-tools.js`](../server/tools/code-tools.js). Symbol ids are **`<repo>:<qualified.name>`** (stable across moves; never line numbers). `grep` remains the always-fresh fallback when the index is cold. **Out of scope (MIN-B5/B7):** Code section UI (MIN-B8), anchor population / `explain_symbol` (MIN-B9). **Tests:** `npm run test:brain`, `test/os/brain-app.test.mts`. Plans: [`MIN-B2`](plans/brain/MIN-B2-wiki-store.md), [`MIN-B3`](plans/brain/MIN-B3-brain-routes-retrieve-adapter.md), [`MIN-B4`](plans/brain/MIN-B4-wiki-tools-prompt.md), [`MIN-B5`](plans/brain/MIN-B5-brain-app-shell.md), [`MIN-B7`](plans/brain/MIN-B7-code-index-backbone.md).
