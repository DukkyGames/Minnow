...
**Config:** `config.json` → `brain.code` (`enabled`, `includeGlobs`, `excludeGlobs`, `repoMapTokenBudget`, `reindexCadence`, `codeEmbeddingsEnabled`); defaults in [`server/brain/code/config.js`](../server/brain/code/config.js), merged in [`loadBrainConfig()`](../server/brain/store.js).
