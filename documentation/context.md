...
**Email app (MIN-126, shipped):** MinnowOS **Email** app (`#/app/email`) — encrypted IMAP account setup ([`server/email/accounts.js`](../server/email/accounts.js); secrets under `~/.minnow/email/secrets/` via `writeEncryptedJsonFile`), read-only folder sync + local cache ([`server/email/imap.js`](../server/email/imap.js), [`server/email/imap-errors.js`](../server/email/imap-errors.js) maps imapflow's generic `Command failed` to provider-aware messages via `responseText`, [`server/email/cache.js`](../server/email/cache.js)), thread grouping ([`server/email/threads.js`](../server/email/threads.js)), AI triage with #13 untrusted wrapping ([`server/email/triage.js`](../server/email/triage.js)), opt-in polling ([`server/email/poller.js`](../server/email/poller.js)), draft + explicit-confirm SMTP send ([`server/email/smtp.js`](../server/email/smtp.js)), agent tools `list_mail` / `draft_reply` ([`server/email/tool-handler.js`](../server/email/tool-handler.js)). **OAuth connect (Google + Microsoft):** shared module [`server/oauth/`](../server/oauth/) — PKCE flows, encrypted tokens at `~/.minnow/oauth/`, BYO client credentials in `config.json` → `oauth` (Settings → **OAuth**); Gmail API + Graph mail via [`server/email/transport.js`](../server/email/transport.js) ([`gmail-api.js`](../server/email/gmail-api.js), [`graph-mail.js`](../server/email/graph-mail.js)); UI [`src/ui/oauth-connect.ts`](../src/ui/oauth-connect.ts) with **Connect with IMAP** on the Email setup/add-account panel (reveals password/IMAP form for Fastmail, iCloud, etc.). Setup: [`documentation/guides/oauth-google.md`](guides/oauth-google.md), [`documentation/guides/oauth-microsoft.md`](guides/oauth-microsoft.md). Plan: [`documentation/plans/oauth-email-calendar.md`](plans/oauth-email-calendar.md). UI: [`src/ui/email-page.ts`](../src/ui/email-page.ts), [`src/ui/email/email-panel.ts`](../src/ui/email/email-panel.ts) — toolbar **Add account** / **Edit account** / **Remove account** when one or more accounts exist (initial setup form when none). Client API: [`src/email/client.ts`](../src/email/client.ts). Styles: [`src/styles/email.css`](../src/styles/email.css). Legacy `#/email` → `#/app/email`. **Limitations:** IMAP/SMTP password path remains for Fastmail/iCloud; Outlook/M365 should use OAuth/Graph. Tests: `npm run test:email`, `npm run test:oauth`.
