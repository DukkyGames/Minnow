...
**Gallery app** (`#/app/gallery`, [`src/ui/gallery-page.ts`](../src/ui/gallery-page.ts)): generate form (prompt, negative prompt, size, provider/model overrides, album select), non-blocking generation status, thumbnail grid with album filter tabs, detail view (full image, metadata, copy prompt, download, delete), disk-usage stats bar + manual cleanup. Panel: [`src/ui/gallery/gallery-panel.ts`](../src/ui/gallery/gallery-panel.ts). API client: [`src/gallery/client.ts`](../src/gallery/client.ts). Styles: [`src/styles/gallery.css`](../src/styles/gallery.css). Persistence: `~/.minnow/gallery/` (MIN-121). Legacy `#/gallery` redirects via [`src/os/router.ts`](../src/os/router.ts). Configure `config.json` → `images.defaultProviderId` to an OpenAI-compatible provider with image API credentials in Settings → Providers.
...
│   ├── tools/
    │   │   ├── definitions.ts      # 60-tool catalog (OpenAI function schemas)
...
~/.minnow/
  ...
  gallery/
    images/                  # generated PNG files (<uuid>.png)
    index.json               # albums + image metadata (JSON v1)
  evals/
