...
**Deep Research (dedicated panel, not a composer mode):** Full-page **`#/research`** (top-bar search-plus icon before Benchmark). **`header.topbar` stays visible**; `#appBody` hidden while `#researchView` is open (same overlay pattern as Benchmark). **Run** tab: query textarea, rounds (Auto/1–20), category (Auto/product/comparison/howto/factcheck), optional search/provider/model overrides, **Start** / **Cancel**, live SVG **synapse** (`src/research/synapse.ts`), result summary + sources, **Open visual report** (Electron preview or new tab — root-relative `/api/research/report/:id` is absolutized against `window.location.origin` before `loadURL` so the guest hits the dev server, not a local `.minnow` file path), **Discuss** (client spinoff: `createAndActivateChat` + user message with report), **Refine** (`POST /start` with `continueFrom`). **Library** tab: search/sort/archived list via `/api/research/library`, open detail, report, discuss, refine, archive, delete. Client: `src/research/{types,client,panel,library,synapse}.ts`, styles `src/styles/research-page.css`. Settings → **Search** + **Deep Research** (`research.json` / `search.json`). The composer **Research** mode was removed in Phase 7 of [`documentation/plans/deep-research-port.md`](plans/deep-research-port.md). The **`researcher`** sub-agent and **Researcher** work agent remain for orchestrator fan-out. **Server:** `server/research/*` (search chain, engine, `store.js`, `routes.js`, `visual-report.js`). Tests: `npm run test:research`; UI: `test/ui/research-*.test.mts`, `test/ui/research-page-html.test.mjs`.
