...
| **`npm run electron:prod`** | `electron/dist/main.js` + in-process server (MIN-111) | Yes (in-process) | Packaged-style run after `npm run build`; requires `server-host` implementation |
