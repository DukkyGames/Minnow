...
| UI | `src/ui/benchmark-page.ts`, `src/ui/benchmark/*` (tabs, overview, charts, tests, `model-run-cards.ts`), `src/styles/benchmark-page.css` — tabs only (OS menubar supplies app chrome); underline tabs aligned with Research; `--mn-*` tokens; `benchmark-mono` / `benchmark-muted` utilities; Overview bench-grid; roster; parallel/sequential schedule; multi-model progress cards; standard toggles; Run tab: flat sections, results strip, history, dense test rows |
