...
**API:** `/api/lsp/status`, `/api/lsp/diagnostics`, `POST /api/lsp/notify` (`{ path, event: open|change|close, text? }` → didOpen/didChange/didClose), `POST /api/lsp/completion` (`{ path, line, character }` → `{ items: [{ label, insertText, kind?, detail? }] }`), `GET/PUT /api/config/lsp` (PUT supports `removeLspIds` for custom server removal). **File viewer:** When LSP is enabled and `npm start` is up, CodeMirror autocomplete calls `src/lsp/completion-client.ts` via `src/ui/file-editor-extensions.ts`; open/edit/save debounce document sync (`src/ui/file-viewer.ts`). **AI inline completion (POLISH-006 / MIN-104):** Optional Copilot-style ghost text after idle debounce (`config.json` → `editorAiCompletion`, default **off**). Uses `createGeneration` + `subscribeToGeneration` with `persist: false` (parsed SSE chunks; reasoning-channel fallback for models that omit `delta.content`). Active chat provider/model (optional overrides). Modules: `src/config/editor-ai-completion.ts`, `src/ui/editor-ai-completion-prompt.ts`, `src/ui/editor-ai-completion-client.ts`, `src/ui/file-editor-ai-extensions.ts` (Tab accept, Esc dismiss; Tab still indents when no ghost). Footer hint: “AI suggestion ready — Tab to accept”. Mounted only for editable buffers (`!readOnlyExcerpt`, not markdown preview). Coexists with LSP dropdown autocomplete. **Settings:** `#/settings/lsp` → **AI inline completion** block (`src/ui/editor-ai-settings.ts`). Plan: [`documentation/plans/Bug Fixes/POLISH-006-editor-ai-autocomplete.md`](plans/Bug%20Fixes/POLISH-006-editor-ai-autocomplete.md). **Catalog:** `src/lsp/defaults.json` ships OpenCode-aligned built-ins (typescript, pyright, rust, …); user overrides in `~/.minnow/lsp.json`. **Settings:** `#/settings/lsp` lists all built-in servers (toggle `disabled`), running/idle badges, and an **Add custom language server** form (`src/ui/lsp-settings.ts`, `src/lsp/config-client.ts`). Test-only `fake` server is hidden in UI. **Tests:** `npm run test:lsp` (fake stdio server for `.fake` files + completion API); `test/ui/editor-ai-completion-prompt.test.mts`, `test/ui/file-editor-ai-keymap.test.mts`.
...
**Sampler (Feature 09):** Settings → **Sampler** (`#/settings/sampler`) holds **global defaults** (`config.json` → `sampler` via `src/config/sampler-meta.ts`, mirrored to composer `#temperature` / `#maxTokens` on load and save) and expandable **per work-agent** / **per sub-agent type** partial overrides (same persistence as before: `work-agents.json`, `sub-agents.json`). UI: `src/ui/settings-sampler.ts`, shared fields: `src/ui/settings-sampler-fields.ts`. Work-agent / sub-agent entity editors no longer embed sampler rows — use this tab instead.

**Thinking mode toggles:** Settings → **Thinking** (`#/settings/thinking`) sets global default **`on`** (`config.json` → `thinking.defaultMode` via `src/config/thinking-meta.ts`) plus per work-agent / sub-agent tri-state overrides (`inherit` | `on` | `off`). Composer `#composerThinkingWrap` (label + `#composerThinkingControl.mode-segmented` with `.mode-segment` buttons, same chrome as `#modeSelector`) writes `Chat.thinkingMode`; explicit chat **on/off** applies to main completions **and** sub-agents for that chat. Resolution: `src/agents/resolve-thinking.ts`; request fields: `src/agents/thinking-to-body.ts` merged in `loop.ts` / `sub-agent-runner.ts`. Catalog `reasoning.allowed_options` on model rows → `ModelCapabilities.reasoningAllowedOptions`. LM Studio may ignore API reasoning controls (one-shot session hint). Title / editor AI / benchmark paths omit per-chat thinking UI (global/off only). Plan: [`documentation/plans/Build out/feature-thinking-mode-toggles.md`](plans/Build%20out/feature-thinking-mode-toggles.md).
