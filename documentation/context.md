...
Completions use `postChatCompletions` with `persist: false` (no chat session pollution). **`llm-driver.ts`** streams via `createSseEventBuffer` / `feedSseEventBuffer` (same `\n\n` framing as chat). **`sse-parse.ts`** emits **every** concatenated JSON object in one `data:` line (llmster / LM Studio glued chunks — not only the first). **`stream-text.ts`** accumulates **assistant prose only** via `BenchmarkStreamTextAccumulator` + `StreamingContentAccumulator` (indexed/cumulative `content` parts); not reasoning/thoughts. **`completion-messages.ts`** injects a direct-reply system prompt, floors `max_tokens` at 256, and retries once when the stream used completion tokens but only the reasoning channel. `extractMessageText` normalizes string or structured parts. Empty streams retry with `tryNonStreamingFallback` and `completionTextFromFallback`. **`cap-multimodal`** uses inline fish photo fixture (`src/benchmark/fixtures/multimodal-probe-fish.png` → `multimodal-probe-image.ts` via `scripts/embed-multimodal-probe-image.mjs`). **`cap-stream`** / **`cap-multimodal`** use `streamCompletionTestDetails` (up to 320-char preview on pass). Distinct from planned Feature 21 eval harness (`~/.minnow/evals/`, multi-model matrix). Plan: [`documentation/plans/benchmark-system-implementation.md`](plans/benchmark-system-implementation.md).
