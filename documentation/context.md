...
**Test board seed (manual dev):** `npm run seed:test-board` — writes a pre-`board_init` orchestrate folder + planner chat into `~/.minnow` sessions (`scripts/seed-test-board.mts`). Presets: `quick` (3 parallel W1 tasks, [`documentation/plans/test-board-quick.md`](../documentation/plans/test-board-quick.md)) or `smoke` (full [`orchestrator-board-smoke.md`](../documentation/plans/orchestrator-board-smoke.md)). Defaults to `fake-board` / `fake-board-model`. Re-run to reset; restart Minnow if it was open.
