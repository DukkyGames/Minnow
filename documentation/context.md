...
**Multi-model campaigns:** `runBenchmarkCampaign()` (`src/benchmark/campaign-runner.ts`) runs integration suites per target via `matrix-scheduler.ts` (concurrency from eval config). Model **roster** in `sessionStorage` (`minnow.benchmark.roster`). **Standard benchmarks** (tiered): bundled mini packs in `src/benchmark/standard/packs/` (MMLU, ARC, GSM8K, HumanEval, TruthfulQA); full tier via `POST /api/benchmarks/datasets` or Tests tab import. Campaign persistence: `GET/POST /api/benchmarks/campaigns`, `GET /api/benchmarks/models` → `~/.minnow/benchmarks/campaigns/`; legacy single-run JSON at `~/.minnow/benchmarks/*.json` unchanged.
...
| UI | `src/ui/benchmark-page.ts`, `src/ui/benchmark/*` (tabs, overview, charts, tests), `src/styles/benchmark-page.css` — AppHeader, tabs, Overview bench-grid, campaign stepper, roster, standard toggles; Run tab: control deck, results strip, history, test rows |
