...
Completions use `postChatCompletions` with `persist: false` (no chat session pollution). **`llm-driver.ts`** streams via `createSseEventBuffer` / `feedSseEventBuffer` (same `\n\n` framing as chat). **`stream-text.ts`** accumulates prose + reasoning deltas into `runOneShot.text`; empty streams retry with `tryNonStreamingFallback` and `completionTextFromFallback`. **`cap-stream`** / **`cap-multimodal`** use `streamCompletionTestDetails` for actionable empty-stream messages (BUG-002 / MIN-96). Distinct from planned Feature 21 eval harness (`~/.minnow/evals/`, multi-model matrix). Plan: [`documentation/plans/benchmark-system-implementation.md`](plans/benchmark-system-implementation.md).
