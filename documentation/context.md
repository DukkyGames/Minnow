...
**Display sleep / AFK liveness:** Electron sets `backgroundThrottling: false` on the main window and uses `powerSaveBlocker` while any board is in auto-run ([`electron/afk-power-guard.ts`](../electron/afk-power-guard.ts), [`src/chat/orchestrate/board-afk-power.ts`](../src/chat/orchestrate/board-afk-power.ts)). On visibility wake or `powerMonitor` resume/unlock-screen, [`reconcileRunningBoardsAfterDisplayWake`](../src/state/orchestrate-board-actions.ts) replays stream-end finalizers for idle task chats and re-drains the queue ([`src/chat/orchestrate/board-display-wake.ts`](../src/chat/orchestrate/board-display-wake.ts)). Task-chat stall heartbeats skip while the page is hidden ([`isSupervisionPageHidden`](../src/agents/controller/wrapper.ts)).
