...
Local Whisper STT + Qwen3-TTS with optional provider/browser fallbacks. Plan: [`documentation/plans/built-in-voice-system.md`](plans/built-in-voice-system.md). UI: **Settings → Audio** ([`src/ui/settings-audio.ts`](../src/ui/settings-audio.ts)) for mic/speaker device routing; **Models → Voice** ([`src/ui/models/voice-panel.ts`](../src/ui/models/voice-panel.ts), `#/app/models/voice`) for runtime health strip, STT/TTS tab panels, unified catalog/installed lists, configuration forms, and test bench. Styles: [`src/styles/models-page.css`](../src/styles/models-page.css) (`.models-voice-*`). Legacy `#/settings/voice` redirects via [`src/os/router.ts`](../src/os/router.ts) and [`src/ui/settings-page.ts`](../src/ui/settings-page.ts). Composer dictation: [`src/ui/composer-voice.ts`](../src/ui/composer-voice.ts); read-aloud: [`src/ui/voice-controls.ts`](../src/ui/voice-controls.ts) — respects `voice.tts.backend` (`local` | `provider` | `browser`) and `setSinkId` when available.
...
| Path | Purpose |
|------|---------|
| `voice/venv/` | Python venv (torch, transformers, soundfile, optional qwen-tts) |
| `voice/meta.json` | Provisioner metadata |
| `voice/installed.json` | Downloaded STT/TTS model manifest |
...
| `POST` | `/api/stt/transcribe` | STT (local or provider) |
| `GET` | `/api/stt/status` | STT backend health |
| `POST` | `/api/tts/synthesize` | TTS (local, provider, or browser) |
| `GET` | `/api/tts/status` | TTS backend health |

**GPU / CPU guidance:** Prefer **CUDA** when `GET /api/system/hardware` reports `backend === 'cuda'` — use `bfloat16` TTS and `whisper-small` STT. The voice provisioner ([`server/voice/provision.js`](../server/voice/provision.js)) installs **CUDA 12.4 PyTorch + matching torchaudio** (`cu124` wheels) on NVIDIA hosts and **CPU-only** builds otherwise; after `qwen-tts` install it **re-pins** torch/torchaudio so pip cannot leave mismatched versions (Windows `WinError 127` on `torchaudio._extension`). Use **Repair** in Models → Voice after upgrading from an older CPU-only install. Runtime status `cudaAvailable` reflects the worker’s actual `torch.cuda.is_available()` when the worker is running, otherwise `torchVariant` from `~/.minnow/voice/meta.json`. On CPU-only hosts, defaults use `float32` and `whisper-base`; expect slower first load and synthesis. **Flash attention** is used when the worker reports `flashAttnAvailable`. Fit badges in UI: [`src/voice/voice-fit.ts`](../src/voice/voice-fit.ts). Client API: [`src/voice/api-client.ts`](../src/voice/api-client.ts); settings forms: [`src/voice/settings-form.ts`](../src/voice/settings-form.ts).
...
Tests: `test/voice/*.test.mjs`, `test/stt/transcribe.test.mjs`, `test/tts/synthesize.test.mjs`, `test/ui/voice-controls.test.mts`, `test/os/models-app.test.mts`.
