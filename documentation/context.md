...
| History + compare | `#benchmarkHistorySelect` loads a saved run into the main panel (summary + suite cards). **Compare** uses the same dropdown as the baseline for regression markers on the current `lastRun`; run a new benchmark after loading history to diff against it. |
