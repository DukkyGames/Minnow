...
**Board task context budget (MIN-39):** Task/test/fixer chats run through [`runChatTurn`](../src/tools/loop.ts) with the active work agent’s **max input tokens** + **context policy** (Settings → Agents → Builder / Tester). Enforcement is skipped when max input tokens is empty (“No cap”) even if a policy is set. Board build chats use mode `build` → default **Builder** agent; tester chats pin `workAgentId: tester`. [`applyContextBudget`](../src/chat/context-budget.ts) trims before each provider send; **summarize** injects `## Prior context (compressed)` for dropped turns. Turn partitioning treats each user seed and each assistant/tool round as separate turns so long single-seed tool loops (typical board tasks) can be summarized instead of only hard-truncating. Separate handoff: [`shouldRouteContinueToNewChat`](../src/state/orchestrate-board-actions.ts) seeds a fresh chat when Continue thresholds are hit (not per-turn trimming).
