...
**Settings → Providers** (`#/settings/providers`): lists registered backends from `GET /api/providers`, **Set active**, **Remove** (when more than one), and **Add provider** (id, label, base URL, API style, auth header, optional API key → `POST /api/providers` + `PUT .../secrets`). Client: [`src/ui/settings-providers.ts`](../src/ui/settings-providers.ts), [`src/providers/store.ts`](../src/providers/store.ts) (`createProvider`, `deleteProvider`, `updateProviderSecrets`).
