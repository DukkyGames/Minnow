...
**Also useful:** [`manual/reference/configuration.md`](manual/reference/configuration.md) (storage layout), [`DESIGN.md`](../DESIGN.md) (visual tokens), [`AGENTS.md`](../AGENTS.md) (agent quick reference), [`plans/`](plans/) (in-flight feature plans).
...
**GitHub Wiki:** [`scripts/publish-github-wiki.mjs`](../scripts/publish-github-wiki.mjs) stages a flattened, link-rewritten public mirror of the **full** published corpus (manual, contributor, guides, maintainer, design system, extensions, roadmap, `context.md`); [`.github/workflows/wiki-sync.yml`](../.github/workflows/wiki-sync.yml) publishes it from `main`. Publication rules and rollback: [`maintainer/wiki-publishing.md`](maintainer/wiki-publishing.md). Plans, specs, archives, and agent memory are not published.
...
**Testing guide:** [contributor/orchestrate-board-testing.md](contributor/orchestrate-board-testing.md) — `npm run test:board`, fake model, `seed:test-board`, `check:board-log`, harness layout. **Manual workflow GUI:** Settings → **Advanced → Board testing** ([`src/ui/settings-board-testing.ts`](../src/ui/settings-board-testing.ts)) — scenario catalog + runner (`GET/POST /api/orchestrate/board-testing/runs/*`), in-process fake model (`POST /api/orchestrate/board-testing/fake-model/*`), seed test board (including `afk` mode), validate board log JSONL. API: [`server/orchestrate/board-testing/`](../server/orchestrate/board-testing/). Shared seed presets: [`src/dev/test-board-seed.ts`](../src/dev/test-board-seed.ts). Unified scenario catalog: [`src/dev/orchestrate-scenarios/`](../src/dev/orchestrate-scenarios/).
...
**Board task context budget (MIN-39):** Task/test/fixer chats run through [`runChatTurn`](../src/tools/loop.ts) with the active work agent’s **max input tokens** + **context policy**. **Global default:** Settings → **Agents** → **Context policy** (`defaultContextEnforcementPolicy` in `sub-agents.json`) applies to all work agents and sub-agent types unless they set their own policy (**Inherit global default** in per-agent settings). Precedence: per-agent user override → global → shipped builtin. Enforcement is skipped when max input tokens is empty (“No cap”) even if a policy is set. Board build chats use mode `build` → default **Builder** agent; tester chats pin `workAgentId: tester`. [`applyContextBudget`](../src/chat/context-budget.ts) trims before each provider send; resolution in [`resolve-context-policy.ts`](../src/chat/resolve-context-policy.ts). **summarize** injects `## Prior context (compressed)` for dropped turns. Turn partitioning treats each user seed and each assistant/tool round as separate turns so long single-seed tool loops (typical board tasks) can be summarized instead of only hard-truncating. Separate handoff: [`shouldRouteContinueToNewChat`](../src/state/orchestrate-board-actions.ts) seeds a fresh chat when Continue thresholds are hit (not per-turn trimming).
