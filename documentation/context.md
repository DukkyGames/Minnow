...
**Migration:** [`server/brain/migrate.js`](../server/brain/migrate.js) — one-time `memory/` → `pages/facts/<slug>.md` (preserves ids). **Memory back-compat:** [`server/memory/{routes,store,middleware}.js`](../server/memory/routes.js) delegate to brain; marked for removal in MIN-B4+. **Out of scope (MIN-B3):** wiki tools, SPA Brain app (MIN-B4/MIN-B5). **Tests:** `npm run test:brain`, `npm run test:memory`. Plans: [`MIN-B2`](plans/brain/MIN-B2-wiki-store.md), [`MIN-B3`](plans/brain/MIN-B3-brain-routes-retrieve-adapter.md).
