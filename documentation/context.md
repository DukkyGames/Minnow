...
| UI | `src/ui/benchmark-page.ts`, `src/styles/benchmark-page.css` — **bench instrument** layout: header lede, bordered **control deck** (preset row + suite toggles), **results strip** (four stat cells, stats-strip style), history toolbar, dense **test rows** (stacked list, meta in trailing column). Summary **TTFT** and **Duration** via `formatDurationMs` (`3.5 s` when ≥1s, else `N ms`). Live progress bar, per-test **running** / **Stopped** states, pass animation on `test-done` |
