...
**Config:** `config.json` → `memory.enabled`, `maxInjectCharsFull` / `maxInjectCharsLite`; `features.memoryInjection` gates retrieval on send (default on). **Client:** `src/memory/client.ts` (`fetchMemoryStatus`, `fetchMemoryEntries`, `retrieveMemoryBlock`, `createMemoryEntry`, …); `src/memory/config.ts` (`shouldInjectMemory`). **Tag parsing (settings form):** `src/memory/parse-tags.ts`. **Settings UI:** `#/settings/memory` — toggle store, live entry count via `GET /api/memory/status`, collapsible **Add memory** form (title, body, comma-separated tags → `createMemoryEntry` / `POST /api/memory/entries` with `source: user`; panel hidden when memory API offline), scrollable list of entries (title, tags, body) via `GET /api/memory/entries?includeBody=1`, per-entry delete, backup/clear actions. **`save_memory`** is enabled by default (permission **ask**). **Tests:** `npm run test:memory`; `test/ui/settings-page-html.test.mjs`, `test/ui/memory-tags-parse.test.mjs`; smoke: `npx tsx scripts/step16-memory-smoke.mjs http://localhost:5173`. Verification: [`documentation/plans/verification/feature-23.md`](plans/verification/feature-23.md).

### LSP integration (Step 17 + feature-02)

Language servers run in Node on `npm start`. Defaults in `src/lsp/defaults.json` (**39** server ids, including test `fake`); user overrides `~/.minnow/lsp.json`.

| Tool | Description |
|------|-------------|
| `get_lsp_diagnostics` | Formatted diagnostics for a relative path |
| `list_lsp_servers` | Configured servers + running state, `requirements`, `disabledReason`, `defaultEnabled` |

**API:** `/api/lsp/status`, `/api/lsp/diagnostics`, `POST /api/lsp/notify` (`{ path, event: open|change|close, text? }` → didOpen/didChange/didClose), `POST /api/lsp/completion` (`{ path, line, character }` → `{ items: [{ label, insertText, kind?, detail? }] }`), `GET/PUT /api/config/lsp` (PUT supports `removeLspIds` for custom server removal). **Settings list source:** `GET /api/config/lsp` → `servers[]` (not `/api/lsp/status` alone). Each `servers[]` item includes `requirements?`, `disabledReason?`, `defaultEnabled?` (install hints are display-only; no disk probe).

**Migration (feature-02):** When `lsp.json` already exists, `loadMergedLspConfig()` in `server/lsp/config-loader.js` runs `migrateLspJsonMissingBuiltins()` — adds `{ disabled }` stubs for any built-in id missing from the user file (never overwrites existing keys; excludes `fake`). Writes `~/.minnow/lsp.json` only when keys were added (idempotent on subsequent loads). Missing file still uses `seedLspJson()`.

**File viewer:** When LSP is enabled and `npm start` is up, CodeMirror autocomplete calls `src/lsp/completion-client.ts` via `src/ui/file-editor-extensions.ts`; open/edit/save debounce document sync (`src/ui/file-viewer.ts`). **Catalog:** OpenCode-aligned built-ins in `defaults.json`; user overrides in `~/.minnow/lsp.json`. **Settings:** `#/settings/lsp` shows **38** visible built-in rows (all except `fake`), built-ins first then custom by label, catalog count hint, per-row extensions, install requirements, combined `disabledReason`, running/idle badges, enable toggles, and **Add custom language server** (`src/ui/lsp-settings.ts`, `src/lsp/config-client.ts`). **Tests:** `npm run test:lsp` — includes `migrate-lsp-json.test.mjs`, `lsp-config-api.test.mjs`, fake stdio + completion API; fixture `test/fixtures/lsp-stale-home/lsp.json`.
...
**Stats strip with editor (feature 26 / E6):** When `#workspaceSplit` has `.viewer-open`, the inference strip in `#mainColumn` uses a **compact** layout: `.stats-expand-btn` visible (Metrics + preview), `.stats-panel` hidden until `.stats-strip.is-expanded` (2-column grid, max-height scroll). `applyFileSidebarVisuals()` calls `syncStatsStripLayoutForViewer()` to collapse expand state when the viewer opens. Styles: [`stats.css`](../src/styles/stats.css), [`file-panel.css`](../src/styles/file-panel.css) (`overflow: hidden` on `.main-column` in split). Verification: [`documentation/plans/verification/feature-26.md`](plans/verification/feature-26.md).
...
**Tests:** `test/file/list-directory-parse.test.mjs`, `test/file/file-tree-boot.test.mjs`, `test/file/file-tree-filter.test.mjs`, `test/file/file-tree-search.test.mjs`, `test/file/file-tree-filter-render.test.mjs`, `test/file/file-viewer-save.test.mjs` (happy-dom + tsx), `test/ui/stats-split-layout.test.mjs`, `test/workspace-ref.test.ts`, `scripts/step-11-smoke.mjs`. Verification: [`documentation/plans/verification/step-11.md`](plans/verification/step-11.md), [`documentation/plans/verification/feature-19.md`](plans/verification/feature-19.md), [`documentation/plans/verification/feature-26.md`](plans/verification/feature-26.md).
...
- **Compact (≤600px):** 16px input (iOS zoom), collapsible stats strip (same expand pattern as **file editor split** on desktop via `.workspace-split.viewer-open`).
...
- **Composer tools (F5 / feature 28):** `#btnComposerTools` in `.input-row` (between attach and textarea) opens `#composerToolsPopover` with the same off/ask/full matrix as Settings ([`src/ui/composer-tools-popover.ts`](../src/ui/composer-tools-popover.ts), [`src/ui/tools-list.ts`](../src/ui/tools-list.ts), compact list `#composerToolsList` without `.tool-desc`). Changes persist via existing `setToolPermission` / `saveToolConfig`; `refreshAllToolListUis()` keeps drawer `#toolsList`, settings `#settingsToolsList`, and composer list in sync. Footer link opens `#/settings/tools`. Styles: [`src/styles/composer-tools-popover.css`](../src/styles/composer-tools-popover.css).
