...
Implementation-ready plans for the 13 selected Odysseus-to-Minnow candidate ports live in [`documentation/plans/Odysseus port/`](plans/Odysseus%20port/). Start with the folder [`README.md`](plans/Odysseus%20port/README.md) for dependency order and Linear tracking ([project](https://linear.app/minnowai/project/odysseus-to-minnow-port-9e0acc5cf9c9)). The first two cross-cutting plans are prompt-injection defense (`odysseus-port-13`) and encrypted secrets (`odysseus-port-12`); downstream integrations should not store new credentials outside the encrypted secret helper once that lands.
