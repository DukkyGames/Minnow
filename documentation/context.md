...
- **`sendMessageWithTools()`** — tool loop capped by **`chat.maxToolTurns`** (default **8**, range 1-64; **Settings > General**, persisted in `config.json` via [`src/config/chat-meta.ts`](../src/config/chat-meta.ts)); streams SSE, `mergeToolCallDelta` / `finalizeToolCalls`, runs enabled tools, appends assistant + tool messages ([`src/tools/loop.ts`](../src/tools/loop.ts)). `MAX_TOOL_TURNS` in code is the default constant only.
