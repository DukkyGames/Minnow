...
**Windows packaging (MIN-113):** `electron-builder` config lives in [`package.json`](../package.json) `"build"` — bundles `dist/**`, `electron/dist/**`, `server/**`, skills manifest script, and `skills-lock.json`; unpacks `node_modules/@lydell/node-pty/**` for PTY; copies `documentation/` as `extraResources`. App id `org.grimmedia.minnow`. Replace [`build/icon.ico`](../build/icon.ico) with a branded asset (must include at least a **256×256** image; electron-builder rejects smaller `.ico` files).
