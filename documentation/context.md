...
| Orchestrate plan control | `#orchestratePlanStrip.orchestrate-plan-control` inline in `#composerControls` after mode segments â€” `src/ui/orchestrate-plan-selector.ts`, `src/styles/orchestrate-plan-selector.css`; refresh on mode/chat/workspace change and when server tools become available (`init-file-panel.ts`); missing `documentation/plans/` maps to `no_plans_dir` in `list-plans.ts` |
...
| Boot | `src/main.ts` â€” `initSubAgentUi()` after `loadSessionsFromStorage()` |
