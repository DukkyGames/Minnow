...
**Thinking mode toggles:** Settings → **Thinking** (`#/settings/thinking`) sets global default **`on`** (`config.json` → `thinking.defaultMode` via `src/config/thinking-meta.ts`) plus per work-agent / sub-agent tri-state overrides (`inherit` | `on` | `off`). Composer `#composerThinkingControl` writes `Chat.thinkingMode`; explicit chat **on/off** applies to main completions **and** sub-agents for that chat. Resolution: `src/agents/resolve-thinking.ts`; request fields: `src/agents/thinking-to-body.ts` merged in `loop.ts` / `sub-agent-runner.ts`. Catalog `reasoning.allowed_options` on model rows → `ModelCapabilities.reasoningAllowedOptions`. LM Studio may ignore API reasoning controls (one-shot session hint). Title / editor AI / benchmark paths omit per-chat thinking UI (global/off only). Plan: [`documentation/plans/Build out/feature-thinking-mode-toggles.md`](plans/Build%20out/feature-thinking-mode-toggles.md).
