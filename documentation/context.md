...
- **`executeTool(name, args, context?)`** — returns `{ content, attachments? }`; browser executor, terminal stream for code tools, or `POST /api/tools`; merges saved `braveApiKey` into `web_search`. **Session result cache** ([`src/tools/result-cache.ts`](../src/tools/result-cache.ts)): after approval/plan guards, read-mostly tools memoize on `(name, normalized-args)` per workspace + `chatId`; mutating tools bust path/git entries; cleared on workspace switch; toggle in Settings → Tools (`toolCache.enabled`, default on). Debug: `localStorage.minnowDebugToolCache = '1'`.
