...
**Model routing (Feature 02):** Settings → **Model routing** (`#/settings/model-routing`) lists per-role provider/model bindings in one pane: work agents (`PUT /api/work-agents/:id`), sub-agent types (`PUT /api/config/sub-agents`), UI Designer `uiDesigner` and chat titles `titles` (`PUT /api/config/meta` via `saveUiDesignerConfig` / `saveTitlesConfig`), and Reef widget LLM on the **active chat** (`reefWidget*` + session save). Catalog: `src/settings/model-routing-catalog.ts`; UI: `src/ui/settings-model-routing.ts`; shared selects: `src/ui/settings-model-binding.ts`. Build plan [`documentation/plans/Build out/feature-02-model-routing.md`](plans/Build%20out/feature-02-model-routing.md); verification [`documentation/plans/verification/feature-02-model-routing.md`](plans/verification/feature-02-model-routing.md).
