...
**Sampler (Feature 09):** Settings → **Sampler** (`#/settings/sampler`) holds **global defaults** only (`config.json` → `sampler` via `src/config/sampler-meta.ts`, mirrored to composer `#temperature` / `#maxTokens`). Per-role sampler overrides moved to **Models** → Advanced. UI: `src/ui/settings-sampler.ts`, fields: `src/ui/settings-sampler-fields.ts`.
