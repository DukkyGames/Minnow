...
| Runner | `src/agents/sub-agent-runner.ts` — headless tool loop; per-type `maxToolTurns` (defaults: `generalPurpose` 16, `explore` 12; overridable in `sub-agents.json`); accumulates `usage` / `stats` on `SubAgentRun` for Orchestrate metrics rollup (MIN-36) |
