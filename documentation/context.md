...
**Archive context policy (MIN-139, Brain-backed):** Fourth policy `archive` offloads stale turns into per-chat Brain wiki pages under `~/.minnow/brain/pages/workspaces/<workspaceKey>/archive/<chatId>/` (`turns-XXXX-YYYY.md` + `entities/`), then collapses bundled ranges to zero-cost `<archived_context turns="…" />` system placeholders in the live API view. **Pre-pass** (main chat only): [`src/chat/archive/index.ts`](../src/chat/archive/index.ts) `applyArchivePolicy()` runs once per send (`turn === 0`) in [`src/tools/loop.ts`](../src/tools/loop.ts) before `applyContextBudget`; later tool turns reuse `applyMemoizedCollapse()`. Staleness: [`staleness.ts`](../src/chat/archive/staleness.ts) (`stalenessTurns`, `pressureThreshold`, `minRecentTurns`). Bundling: server [`server/brain/archive.js`](../server/brain/archive.js) (`POST /api/brain/archive`, `GET /api/brain/archive/status`, `DELETE /api/brain/archive/chat/:id`) via Brain `llmCall` + page writer (extractive fallback); quote validation in [`bundle-validate.ts`](../src/chat/archive/bundle-validate.ts). **Auto-prelude:** when the last message is `user`, scoped `POST /api/brain/retrieve` with `scope.chatId` injects `<retrieved_context source="archive">` ([`auto-prelude.ts`](../src/chat/archive/auto-prelude.ts)); retrieve filter in [`server/brain/retrieve.js`](../server/brain/retrieve.js). **Recall tools** (browser, permission `ask`): `recall_chat_context`, `recall_turn_full` ([`recall-chat-context.ts`](../src/tools/recall-chat-context.ts), [`recall-turn-full.ts`](../src/tools/recall-turn-full.ts)). `chat.lastContextTrim` stats chip in metrics strip preview ([`stats.ts`](../src/ui/stats.ts)). Chat delete → fire-and-forget archive folder cleanup ([`cleanup.ts`](../src/chat/archive/cleanup.ts)). Sub-agents with `archive` are normalized to `slide` at load. Tests: [`test/chat/archive/`](../test/chat/archive/), [`test/brain/archive-routes.test.mjs`](../test/brain/archive-routes.test.mjs). Linear: [MIN-139](https://linear.app/minnowai/issue/MIN-139/archive-context-token-reduction-policy-backed-by-brain).
