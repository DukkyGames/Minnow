...
**Model routing (Feature 02 — planned):** Consolidated `#/settings/model-routing` will list per-role provider/model bindings (work agents, sub-agent types, UI Designer `config.json` `uiDesigner`, chat titles `titles`, Reef widget LLM on the active chat) using existing APIs only — build plan [`documentation/plans/Build out/feature-02-model-routing.md`](plans/Build%20out/feature-02-model-routing.md); roadmap [#2](plans/feature-audit-roadmap.md#2-per-agent-model-routing--built-formalize).
...
| Bug tracker (MIN-16) | `Chat.bugBoard?: BugBoardState` in **debug** mode (`modeId: debug`, UI label **Bugs**). Columns: Reported → Investigating → Planned → Fixing → Complete. Store: `src/state/bug-board-store.ts`, events: `bug-board-events.ts`. Tools: `bug_add`, `bug_update`, `bug_get_state` (`src/tools/bug-board-tools.ts`). UI: `src/ui/bug-board.ts`, `src/styles/bug-board.css` (reuses `.board-*` layout); `renderChatFromHistory` when `viewMode === board`. Pipeline: `src/chat/bug-board/pipeline.ts` — **Investigate** (`debugger` sub-agent), **Plan fix** (`bug-planner` + `workAgentId: planner` → `documentation/plans/bugs/<id>.md`), **Start fix** (sets `orchestratePlanPath`, switches to Orchestrate + board kickoff). Sub-agents: `debugger`, `bug-planner` in `sub-agents.json` + `src/agents/prompts/sub-agents/`. Prompts: `modes/debug.*.md`. **Global list:** `src/state/global-bugs.ts` (`collectGlobalBugs`), `src/ui/global-bugs-page.ts` + `#/bugs` route; **All bugs** icon `#btnAllBugs` in `.chat-sidebar-footer` (open-count badge); filters (workspace scope, column, hide complete); row → `openGlobalBugInChat`. Tests: `test/state/bug-board-store.test.mts`, `test/tools/bug-board-tools.test.mts`, `test/state/global-bugs.test.mts`. View toggle shares Orchestrate `view-mode-toggle` (`isBoardViewActive` for composer hide). Plan: [`documentation/plans/min-16-global-bugs.md`](plans/min-16-global-bugs.md). |
