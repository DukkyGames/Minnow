...
| Persistence | `GET/POST /api/benchmarks`, `GET /api/benchmarks/:id` → `~/.minnow/benchmarks/`; `localStorage` fallback (`minnow.benchmarks.history`, cap 5) when the server is unreachable or POST fails. `listRuns()` merges server + local summaries (server wins on duplicate ids). |
| History + compare | `#benchmarkHistorySelect` loads a saved run into the main panel (summary + suite cards). While a run is **in progress**, browsing history does not overwrite `lastRun` or `liveRunDrawerMeta`; progress keeps updating in memory and the **Current run** button (`#btnBenchmarkBackToCurrent`) plus a **Current run (in progress)** history option restore the live panel. **Compare** uses the same dropdown as the baseline for regression markers on the active panel (`lastRun` when idle, live cards when running). |
