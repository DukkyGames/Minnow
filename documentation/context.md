...
Multi-provider registry: `~/.minnow/providers/`. UI: Models app → Providers. Chat uses composite model keys (`providerId` + model id) in [`src/lib/model-select-key.ts`](../src/lib/model-select-key.ts). The top-bar picker and composer model menus share the same catalog via [`src/ui/model-select-picker.ts`](../src/ui/model-select-picker.ts); composer menus resolve **Load/Unload** against the **active chat model** (not the global menubar default) via [`src/ui/model-host-filter-context.ts`](../src/ui/model-host-filter-context.ts). **`fetchModels()`** also merges **My Models** loadable GGUF rows into a **My Models** optgroup ([`src/models/model-select-library.ts`](../src/models/model-select-library.ts)) with synthetic provider id `minnow-library`; Load/Unload starts or stops Minnow serves from the picker, and chat turns auto-load library bindings before the first completion. `fetchModels()` refreshes the global `#modelSelect` only — it does **not** copy the default onto the active chat (per-chat bindings come from the composer or send-time `resolveEffectiveChatModelBinding`). Desktop hero/docked composer picks are copied onto the desktop sandbox chat when the first send bootstraps or seeds a new thread ([`src/os/desktop-chat.ts`](../src/os/desktop-chat.ts) `ensureReadyForSend` / `createFreshAssistantChat`). On send, if the active model is not loaded on a load/unload-capable provider (e.g. LM Studio), the turn loads it first and the assistant stream row shows **Loading model…** ([`src/ui/stream-status.ts`](../src/ui/stream-status.ts)) until generation begins (**Generating response…**).
