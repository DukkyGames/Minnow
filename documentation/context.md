...
- **Inbox productivity:** the stream marker hosts a list toolbar (master tri-state checkbox, selection actions, Unread-scope **Read all**, search, keyboard help). Rows use listbox/option semantics with persistent checkboxes, quick star, context menu (`email-context-menu.ts`), and private drag payloads (`application/x-minnow-email-threads`) onto rail folders. Selection helpers live in [`src/ui/email/email-selection.ts`](../src/ui/email/email-selection.ts). Thread summaries include folder-scoped `messageIds` for bulk ops. `POST /api/email/messages/bulk` accepts `spam` (junk-folder role resolution). `POST /api/email/accounts/:id/messages/mark-all-read` marks every unseen message in a folder (optional FTS `query`) in chunks and returns `{ attempted, updated, failed }`.
