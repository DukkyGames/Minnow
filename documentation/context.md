...
Unified **Benchmarking** app at `#/benchmark` / `#/app/bench` (OS **Benchmarking** app). Tabbed UI: **Overview** (multi-model comparison grid with animated tok/s and quality bars, MinnowOS-style), **Charts** (SVG bar/radar/history), **Tests** (integration + standard + custom catalog, prompt drawer, pack editor, full-tier JSON import), **Run** (model roster, integration suite toggles, standard mini/full packs, campaign stepper). **`header.topbar` stays visible** while open.

**Multi-model campaigns:** `runBenchmarkCampaign()` (`src/benchmark/campaign-runner.ts`) runs integration suites per target via `matrix-scheduler.ts` (concurrency from eval config). Model **roster** in `sessionStorage` (`minnow.benchmark.roster`); Run tab adds models via provider/model dropdowns (`roster-picker.ts`, same catalog as Settings) plus **Add active** from the top-bar model. **Standard benchmarks** (tiered): bundled mini packs in `src/benchmark/standard/packs/` (MMLU, ARC, GSM8K, HumanEval, TruthfulQA); full tier via `POST /api/benchmarks/datasets` or Tests tab import. Campaign persistence: `GET/POST /api/benchmarks/campaigns`, `GET /api/benchmarks/models` → `~/.minnow/benchmarks/campaigns/`; legacy single-run JSON at `~/.minnow/benchmarks/*.json` unchanged.

**Settings → Evals** redirects to `#/app/bench/tests` (eval APIs under `/api/evals/*` unchanged; custom packs still in `~/.minnow/evals/packs/`).

**Run tab** retains integration **control deck** (Quick / Full, suite toggles, Run/Stop), progress bar, results strip, history, suite test cards, transcript drawer. Session resume (`active-run-session.ts`) uses legacy single-model path when resuming.
...
| UI | `src/ui/benchmark-page.ts`, `src/ui/benchmark/*` (tabs, overview, charts, tests), `src/styles/benchmark-page.css` — tabs only (OS menubar supplies app chrome); underline tabs aligned with Research; `--mn-*` tokens; `benchmark-mono` / `benchmark-muted` utilities; Overview bench-grid; campaign stepper; roster; standard toggles; Run tab: control deck, results strip, history, test rows |
...
| Standard LLM packs | `src/benchmark/standard/` (`packs/*.json`, `scorers.ts`, `runner.ts`, `catalog.ts`); import script `scripts/fetch-benchmark-dataset.mjs` |
| Persistence | `GET/POST/DELETE /api/benchmarks`, `GET /api/benchmarks/:id` (legacy runs); `GET/POST /api/benchmarks/campaigns`, `GET /api/benchmarks/campaigns/:id`, `GET /api/benchmarks/models`, `POST /api/benchmarks/datasets` → `~/.minnow/benchmarks/`; `localStorage` fallback for campaigns (`minnow.benchmarks.campaigns`, cap 10) and runs (`minnow.benchmarks.history`, cap 5) |
