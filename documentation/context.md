...
| Tools | `issue_add` / `issue_update` / `issue_link` / `issue_get_state` ([`issue-tools.ts`](../src/tools/issue-tools.ts)). **`bug_add` / `bug_update` / `bug_get_state`** remain as thin aliases in [`bug-board-tools.ts`](../src/tools/bug-board-tools.ts) for one more release (column/severity ↔ status/priority). Allowed in **General**, **Build**, **Plan**, **Debug**, and **Desktop** ([`tool-groups.ts`](../src/chat/modes/tool-groups.ts) `issues` + `bug-board` groups). |
| Workflows | **Send to chat** / **Send to background** mode dropdowns (General, Build, Plan, Debug foreground; Debug + Plan background sub-agents) / **Send to board** ([`src/chat/issues/pipeline.ts`](../src/chat/issues/pipeline.ts), [`workflow-seeds.ts`](../src/chat/issues/workflow-seeds.ts), [`issues-workflow-menu.ts`](../src/ui/issues-workflow-menu.ts)); triage **Expand with agent** (detail panel + board cards) → shipped `issue-writer`; detail **Open plan** foregrounds Code then opens the plan in the file viewer (`openIssuePlanInEditor`) |
...
- **~89 built-in tools** — [`src/tools/definitions.ts`](../src/tools/definitions.ts)
