...
**Email app (MIN-126 + agent redesign):** MinnowOS **Email** app (`#/app/email`) — **agent-first** UX: default **inbox dashboard** ([`src/ui/email/email-dashboard.ts`](../src/ui/email/email-dashboard.ts)) with digest headline, stats-strip-style triage instrumentation, narrative brief (72ch), divider-based **attention queue** rows (click-through to thread), and **quick reply chips** (send / reprompt); **three-pane mail view** ([`src/ui/email/email-layout.ts`](../src/ui/email/email-layout.ts)) with **Compose** CTA in folder sidebar (unread badges), icon bulk toolbar (read/unread/flag/archive/delete + search + pagination), list rows with avatars, inline star toggle, attachment icon, and urgency badges; **reading pane** with prominent Reply / Reply all / Forward actions, flag/archive/delete + overflow menu (mark unread, mark spam, move to folder), attachment chips, and **formatted HTML bodies** ([`src/ui/email/email-body.ts`](../src/ui/email/email-body.ts) — DOMPurify render of cached `bodyHtml`, plain-text fallback) plus **inline compose** ([`src/ui/email/email-compose.ts`](../src/ui/email/email-compose.ts) — reply modes, Cc on reply-all/forward, quick-draft chips); shared mail icons in [`src/ui/email/email-icons.ts`](../src/ui/email/email-icons.ts); styles in [`src/styles/email.css`](../src/styles/email.css) (`--mn-*`, flat chrome, mail list scroll via `.email-layout` flex chain + `.email-list-rows` overflow, ≤900px list↔reader swap with back button); **automations** builder ([`src/ui/email/email-automations.ts`](../src/ui/email/email-automations.ts)). Orchestrator: [`src/ui/email/email-panel.ts`](../src/ui/email/email-panel.ts). Server: encrypted accounts ([`server/email/accounts.js`](../server/email/accounts.js)), IMAP/Gmail/Graph sync ([`server/email/transport.js`](../server/email/transport.js)), **mail mutations** ([`server/email/imap-actions.js`](../server/email/imap-actions.js), [`server/email/mail-actions.js`](../server/email/mail-actions.js) — read/unread, flag, archive, move, delete, bulk), cache flags + inbox summary + reply variants + sanitized `bodyHtml` ([`server/email/cache.js`](../server/email/cache.js), [`server/email/sanitize-html.js`](../server/email/sanitize-html.js); re-sync hydrates older rows), **agent layer** ([`server/email/agent.js`](../server/email/agent.js) — digest, variants, post-sync hooks), **SSE events** ([`server/email/events.js`](../server/email/events.js) — `GET /api/email/events`), **automations** ([`server/email/automations.js`](../server/email/automations.js) at `~/.minnow/email/automations.json`), poller with auto-triage ([`server/email/poller.js`](../server/email/poller.js)), LLM-assisted draft + explicit-confirm send ([`server/email/smtp.js`](../server/email/smtp.js)). REST: `/api/email/accounts/:id/summary`, message flags/move/archive/bulk, reply-variants + send. Agent tools: `list_mail`, `draft_reply`, `summarize_inbox`, `generate_reply_variants`, `email_action` ([`server/email/tool-handler.js`](../server/email/tool-handler.js)). OAuth: [`server/oauth/`](../server/oauth/), [`src/ui/oauth-connect.ts`](../src/ui/oauth-connect.ts). Client: [`src/email/client.ts`](../src/email/client.ts), [`src/email/client-ext.ts`](../src/email/client-ext.ts). Plan: [`documentation/plans/email-agent-redesign.md`](plans/email-agent-redesign.md). **No auto-send** — user always confirms send (MIN-126 policy). Tests: `npm run test:email`.
