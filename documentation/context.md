...
| **Preview browser (MIN-105)** | `src/ui/preview-panel.ts` — right split pane `#previewPane` (mutually exclusive with `#fileViewerPane` in v1). Serves workspace HTML/assets via `GET /api/preview/file/*` ([`server/preview/middleware.js`](../server/preview/middleware.js), same `resolveSafePath` as tools; blocks `node_modules`, `dist`, `.git`, `.vite`, `.minnow`; caps concurrent streams). Iframe load on session restore is deferred until `window` `load` + idle so cloud shells do not compete with Vite boot. Closing the panel sets `about:blank` to release sockets. Arbitrary `http(s)://` URLs use a sandboxed iframe (`allow-scripts allow-forms allow-popups allow-modals`; no `allow-same-origin`). Topbar `#btnPreviewToggle`, file-tree **Open in preview** for `.html`, address bar + reload + **Auto-reload** (250ms debounce on `emitFileSaved` from viewer `save_file`). Persisted: `filePanel.rightPaneMode`, `previewSource`, `previewAutoReload`. Distinct from CDP screencast mirror (POLISH-011). |
