...
Persistent notes under `~/.minnow/memory/` (`index.json` + `entries/<uuid>.md`). Injected via composer `memory` part (`src/chat/prompts/memory/*.md` wraps `{{memory}}` around the retrieved block) when enabled; retrieved blocks are **fenced as untrusted** (`source="memory"`) in [`server/engine/retrieve.js`](../server/engine/retrieve.js) (re-exported via `server/memory/retrieve.js`). **Shared vector engine (MIN-B1):** generic embedding, vector index, hybrid retrieve, proposals, backup, and vector-sync live under [`server/engine/`](../server/engine/) with disk paths injected as `{ rootDir, vectorsPath, proposalsPath, backupsDir }`; [`server/memory/engine-paths.js`](../server/memory/engine-paths.js) resolves those from `getMemoryDir()` and thin adapters in `server/memory/{embeddings,vector-store,retrieve,proposals,backup,vector-sync}.js` bind the memory feature. **Retrieve:** keyword scoring remains the default; optional **hybrid keyword + cosine** retrieval (`retrieveMemoryBlockHybrid` in [`server/engine/retrieve.js`](../server/engine/retrieve.js)) blends normalized keyword and vector scores (`final = (1-w)*keyword + w*cosine`, default `w=0.5`). When embeddings are disabled, unhealthy, or time out, retrieval falls back to keyword-only. **Vector sidecar:** `~/.minnow/memory/vectors.json` ([`server/engine/vector-store.js`](../server/engine/vector-store.js)) stores per-entry vectors; CRUD hooks in [`server/engine/vector-sync.js`](../server/engine/vector-sync.js) upsert/delete asynchronously without blocking saves. **Local embedder:** lazy-loaded `@xenova/transformers` (`Xenova/all-MiniLM-L6-v2` default) caches models under `~/.minnow/models/embeddings/` ([`server/engine/embeddings.js`](../server/engine/embeddings.js); sets `HF_HUB_DISABLE_SYMLINKS=1` on Windows). **Provider embedder:** optional OpenAI-compatible `POST /v1/embeddings` via configured `providerId` + `embeddingsPath` (default in [`server/providers/paths.js`](../server/providers/paths.js)). **Retrieve fallback:** keyword queries with no token matches still inject recent/pinned entries. **Sub-agents:** `buildSubAgentSystemPrompt` in [`src/agents/sub-agent-prompt.ts`](../src/agents/sub-agent-prompt.ts) injects the same memory block (task text as query) and `save_memory` guidance when the tool is allowed. **Save guidance:** full/lite base prompts + memory templates tell the model to call `save_memory` for explicit “remember this” requests and stable preferences/facts (not secrets or one-off state).
...
### Brain wiki store (MIN-B2 — CORTEX storage layer)

On-disk wiki at `~/.minnow/brain/` — nested markdown pages with YAML frontmatter as source of truth; `catalog.json` is a **rebuildable cache** (never authoritative). Bootstrap runs from [`ensureMinnowLayout()`](../server/config/home.js) via dynamic `ensureBrainStore()` ([`server/brain/store.js`](../server/brain/store.js)). **Layout:** `index.md`, `log.md`, `schema.md`, `catalog.json`, `state.json` (migration flag), `vectors.json` / `proposals.json` (engine sidecars keyed by page **UUID** `id`), `pages/{facts,<domain>,workspaces/<key>}/`, plus empty `sources/` and `code/` (MIN-B7). **Paths + sandbox:** [`server/brain/paths.js`](../server/brain/paths.js) — `resolvePagePath(relPath)` rejects absolute paths, `..`, leading slashes, drive letters, null bytes, non-`.md` extensions, and symlink escapes (`fs.realpath` containment under `pages/`). **CRUD:** [`server/brain/store.js`](../server/brain/store.js) — `createPage`, `readPage`, `updatePage`, `deletePage`, `listPages`, `getPageTree`, `appendLog`, `rebuildIndex`, `rebuildCatalog`; path-based wikilinks `[[folder/slug]]` in `links[]`; no `MAX_BODY_BYTES` / `MAX_ENTRIES` caps. Every write appends to `log.md` and schedules vector sync via [`server/brain/vector-sync.js`](../server/brain/vector-sync.js) (adapter over [`server/engine/vector-sync.js`](../server/engine/vector-sync.js); paths from [`server/brain/engine-paths.js`](../server/brain/engine-paths.js)). **Migration:** [`server/brain/migrate.js`](../server/brain/migrate.js) — one-time idempotent `memory/` → `pages/facts/<slug>.md` (preserves `id`, `createdAt`, `updatedAt`), copies `vectors.json` + `proposals.json` verbatim, archives old `memory/` under `backups/` ([`server/memory/backup.js`](../server/memory/backup.js)), sets `state.json` → `migratedFromMemory`. **Out of scope (MIN-B2):** HTTP routes, tools, UI — see [`documentation/plans/brain/MIN-B3-brain-routes-retrieve-adapter.md`](plans/brain/MIN-B3-brain-routes-retrieve-adapter.md). **Tests:** `npm run test:brain`. Plan: [`documentation/plans/brain/MIN-B2-wiki-store.md`](plans/brain/MIN-B2-wiki-store.md).
