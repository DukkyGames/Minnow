...
**Chat code change stats:** File-mutation server tools (`save_file`, `append_file`, `insert_at_line`, `replace_text_in_file`, `delete_path` for files only, `copy_file`, `move_file`) return optional `codeChange` `{ additions, deletions, path? }` from [`server/tools/line-diff-stats.js`](../server/tools/line-diff-stats.js) via [`server/runtime/tools-middleware.js`](../server/runtime/tools-middleware.js). [`src/tools/client.ts`](../src/tools/client.ts) records into `chat.codeChangeTotals` (`recordCodeChange` in [`src/usage/code-change-ledger.ts`](../src/usage/code-change-ledger.ts)) when `context.chatId` is set — sub-agents pass `parentChatId`, so edits roll up to the parent chat. Tool rows persist `codeChange` on `ToolResultMessage`; UI badges in [`src/ui/tool-messages.ts`](../src/ui/tool-messages.ts); cumulative strip `#codeChangeStrip` above the composer ([`src/ui/code-change-strip.ts`](../src/ui/code-change-strip.ts)). Manual file-tree CRUD passes active `chatId` ([`src/ui/file-tree-ops.ts`](../src/ui/file-tree-ops.ts)). Plan: [`documentation/plans/chat-code-change-tracking.md`](plans/chat-code-change-tracking.md).

**Settings global finder:** `.settings-page-header` includes `#settingsSearchInput` (registry-driven index in `src/ui/settings-search-index.ts`, ranking in `settings-search-rank.ts`, navigation in `settings-search-navigate.ts`, UI in `settings-search-finder.ts`). **Ctrl/Cmd+K** focuses the finder while settings is open (skipped when `#settingsPromptsHubSearch` is focused). Choosing a result opens the section via existing hash routing, awaits `refreshSettingsSection`, then scrolls to `data-settings-search-key` (tool rows/categories, toggle rows, groups) or falls back to the section’s first `.settings-group`. Does not mirror queries into the Prompts hub local search.
...
**Expert Lab:** Pick grid → brief → run timeline, plus **Make your own expert** (create), and **Edit** / **Delete** on user-owned tiles (⋯ menu). Custom experts are generated via one-shot LLM ([`create-expert.ts`](../src/chat/experts/create-expert.ts) + [`expert-creator`](../src/chat/prompts/info/expert-creator.full.md) info prompt), saved under `~/.minnow/prompts/experts/<id>/expert.{full,lite}.md`, and registered through [`syncExpertRegistryFromServer()`](../src/chat/experts/registry.ts) (also called from [`init-prompts.ts`](../src/chat/prompts/init-prompts.ts)). User-owned = `source: user` and id not in the built-in bundle ([`isUserOwnedExpert`](../src/chat/experts/expert-ownership.ts)); no Auto-routing keywords. Create success lands on **brief**; edit/delete return to pick. APIs: existing `PUT/DELETE /api/prompts/experts/:id/prompt` (+ `?raw=1` for full file reads), `DELETE /api/prompts/experts/:id` removes the entity directory. Runs use `runChatTurn` with `composedSystemPromptOverride` = expert `fullBody`; stream hooks in `src/ui/expert-lab-stream.ts` (called from `src/tools/loop.ts`). Phases: Understanding, Clarifying (`ask_question` — `notifyExpertLabToolRound` runs **before** `executeTool` so `#questionHost` reparents into `#expertLabClarifyingBody` before the modal opens), Working (token count + spinner), Output (live markdown via debounced `scheduleAssistantBubbleRender` on `#expertLabOutputBubble` from `onPartialText` in [`expert-lab-page.ts`](../src/ui/expert-lab-page.ts); final pass `setAssistantBubbleContent` on `onRunEnd`). Run step header **Open chat** calls `openExpertLabInChatView()` (hides Expert Lab, keeps `EXPERT_LAB_CHAT_ID` active); `isStreamDomVisible` allows chat-area streaming when `expertLabPageOpen` is false. Sidebar filters `kind: 'expert-lab'`. Meta: optional `icon` + `accent` (`sage` \| `amber` \| `cyan` \| `coral` \| `violet` \| `rose`) in expert front matter.
...
**Tests:** `test/experts/**/*.test.mjs`, `test/chat/experts/*.test.mjs`, `test/ui/expert-lab-custom-expert.test.mjs`. Verification: [`documentation/plans/verification/step-06.md`](plans/verification/step-06.md).
...
**Welcome screen (POLISH-016):** When `GET /api/workspace` reports `isDefault: true` (workspace still equals the Minnow app root), boot opens [`src/ui/welcome-page.ts`](../src/ui/welcome-page.ts) at `#/welcome` instead of the chat shell (`#appBody` hidden). **Open project** uses the in-app folder picker; **Create new project** is an inline wizard (name → `POST /api/workspace/mkdir` under `newProjectParent`, usually `~/Projects`); **Recent workspaces** mirrors the top-bar MRU. **Continue in Minnow folder** dismisses welcome for the session only. Full-page routes (`#/settings`, `#/bugs`, `#/benchmark`, `#/experts`) skip auto-welcome on boot. Top bar stays visible with `.topbar--welcome` (brand + Settings only).
...
**Tests:** `test/workspace/workspace-api.test.js`, `test/workspace/safe-path.test.js`, `test/ui/workspace-recent-menu.test.mjs`, `test/ui/welcome-page.test.mjs`. Verification: [`documentation/plans/verification/feature-04.md`](plans/verification/feature-04.md).
...
| Viewer | `src/ui/file-viewer.ts` — `read_file` / `read_file_range` / `save_file`; **workspace images** open as read-only **image preview**; chat attachment snapshots; **`.md` / `.markdown`** GFM preview by default with right-click **Open as code** / **Open as preview**; selection right-click **Add selection to chat** / **Quick edit** (Phase 4); CodeMirror 6 + `editor-core-extensions.ts` + language-data; Save + Ctrl/Cmd+S; large files (>512 KB) read-only excerpt; LSP completions; optional AI ghost + Mod-K quick edit when server up |
