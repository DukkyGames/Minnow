...
On-disk wiki at `~/.minnow/brain/` — nested markdown pages with YAML frontmatter as source of truth; `catalog.json` is a **rebuildable cache** (never authoritative). Bootstrap: `ensureBrainStore()` from [`server/brain/store.js`](../server/brain/store.js) (called by `initBrainApi()` in [`server/runtime/bootstrap.js`](../server/runtime/bootstrap.js)). **Layout:** `index.md`, `log.md`, `schema.md`, `catalog.json`, `state.json`, `vectors.json` / `proposals.json` (engine sidecars keyed by page **UUID** `id`), `pages/{facts,<domain>,workspaces/<key>}/`, `sources/` (immutable ingest), `code/` (MIN-B7). **Paths + sandbox:** [`server/brain/paths.js`](../server/brain/paths.js) — `resolvePagePath(relPath)` on every HTTP page route. **CRUD:** [`server/brain/store.js`](../server/brain/store.js). **Workspace-scoped retrieve:** [`server/brain/retrieve.js`](../server/brain/retrieve.js) — `scopePagesToWorkspace()` filters global pages + `workspaces/<key>/**` before calling pure [`server/engine/retrieve.js`](../server/engine/retrieve.js). **Synthesis / ingest / lint:** [`server/brain/synthesis.js`](../server/brain/synthesis.js) (default `requireConfirmation: false` → writes `pages/facts/` with `source: synthesis`), [`server/brain/ingest.js`](../server/brain/ingest.js), [`server/brain/lint.js`](../server/brain/lint.js) (orphans, stale, missing links; anchor drift extension point MIN-B9). **HTTP:** [`server/brain/routes.js`](../server/brain/routes.js) + [`server/brain/middleware.js`](../server/brain/middleware.js) registered in [`server/runtime/middlewares.js`](../server/runtime/middlewares.js).
