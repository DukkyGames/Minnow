...
**Multi-device sync (MIN-354 Phase 0 / MIN-357):** Landed. [`src/state/session-sync.ts`](../src/state/session-sync.ts) subscribes to `GET /api/session/stream` (SSE snapshot + patch with monotonic `rev`) and reconciles remote state on thin clients. On `EventSource` CLOSED (e.g. server restart), it refreshes the per-boot token like [`install-fetch-auth.ts`](../src/api/install-fetch-auth.ts) and reconnects when the token changes (capped + backed off; unchanged token does not loop). While a client is locally mirroring a stream, remote patches are **buffered** (not discarded) and applied when idle — bumping `rev` without applying would permanently miss one-shot SSE events. `PUT` and `PATCH` `/api/config/sessions` accept optional `If-Match: <rev>` and/or body `expectedRev`/`baseRev` (409 + `X-Session-Rev` on conflict; schema `baseVersion` is unrelated). Client saves retry once after conflict; keepalive PUT also sends `If-Match` when known. The Phase 0 **board driver lease** (`POST /api/session/lease`) was **removed in Phase 4** — the Session Engine is the sole driver by construction (endpoints return `410 LEASE_REMOVED`). Optional `MINNOW_TOKEN` accepts Bearer or `?minnowToken=` (EventSource). Server modules: [`server/session/`](../server/session/) (not `server/engine/`, which is vector-store). Plan: [`plans/server-session-engine.md`](plans/server-session-engine.md).

**Server Session Engine (MIN-354 Phase 1–4 / MIN-359…MIN-362):** **Default ON** at `npm start` (no env var required). Emergency opt-out: `MINNOW_SERVER_ENGINE=0|false|off|no`. Composer main-chat sends dispatch `POST /api/session/commands` (`send_message` / `stop_generation` / `steer_message` / `enqueue_message` / `answer_question`) instead of renderer `runChatTurn`; the engine owns an in-memory SessionState, runs a DOM-less tool loop ([`src/session-engine/main-chat-loop.ts`](../src/session-engine/main-chat-loop.ts)), creates `/api/generations` streams, and publishes committed history via Phase 0 SSE. **Load path:** [`server/session/engine-module.js`](../server/session/engine-module.js) picks tsx (dev / `npm test`) or the pre-built [`engine-bundle.mjs`](../scripts/build-engine-bundle.mjs) (packaged Electron; built by `npm run build:engine-bundle` / `electron:build`). Boot is shared via [`server/runtime/engine-boot.js`](../server/runtime/engine-boot.js) from `server.js` and [`electron/server-host.ts`](../electron/server-host.ts). `GET /api/session/engine` reports `enabled` only when the flag is on **and** the module is available; otherwise clients fall back to the renderer loop. Composer follow-ups drain in [`commands.js`](../server/session/commands.js) **after** `endEngineTurn` (skip on abort / pending steer); failed turns write a single `Error:` history row from the command handler. **Reasoning persistence:** engine turns accumulate reasoning SSE via [`engine-reasoning-capture.ts`](../src/session-engine/engine-reasoning-capture.ts) and commit `thinking` / `thinkingDurationMs` (and `thinkingSignature` on tool-call rows) to `Chat.history` so Phase 0 reconcile + [`renderChatFromHistory`](../src/ui/messages.ts) keep Thoughts toggles after the live mirror tears down. **Live transcript paint:** Phase 0 reconcile calls [`renderChatInForegroundShell`](../src/ui/messages.ts) (desktop `#desktopChatCol`, Chat app `#chatAppMessageCol`, Email assistant column, or Code `#chatArea`) — not always `#chatArea` — so tool-call rows and committed history appear on the visible shell during engine turns; token streaming still mirrors via [`engine-stream-mirror.ts`](../src/chat/engine-stream-mirror.ts). Composer stop/queue affordances treat `chat.engineTurnActive` as busy via [`isActiveChatBusy`](../src/chat/streaming-state.ts). **Chat auto-titles:** engine sends never reach renderer `runChatTurn`'s title `finally`, so composer / programmatic engine paths call [`scheduleChatTitleAfterEngineTurn`](../src/chat/titles/schedule.ts) (wait for idle, then the usual title job); Super Plan / plan-screen schedule after `dispatchSendMessageAndAwaitIdle`. Onboarding kickoff passes `shouldScheduleTitle: false`. **Attachments / VLM:** `send_message` accepts serializable `attachments` (same `Attachment` shape as the composer); history persists via [`buildHistoryUserContent`](../src/attachments/api-content.ts) placeholders/file blocks; in-memory `turnAttachments` rebuild multimodal `image_url` parts for vision models ([`build-api-messages.ts`](../src/session-engine/build-api-messages.ts)). **ask_question:** engine parks the tool call, soft-publishes `chat.pendingAskQuestion`, and resumes on `answer_question`; the renderer opens the existing strip from that hint ([`engine-ask-question-mirror.ts`](../src/chat/engine-ask-question-mirror.ts)). **Phase 2 (MIN-360):** orchestrate board scheduler runs in the engine — commands `board_init` / `board_start` / `board_stop` / `board_start_task` / `board_requeue_task` / `board_set_autonomy` / `board_run_final_test` / `board_recover_task` / `set_model`; stream-end bus is in-process ([`src/session-engine/stream-bus.ts`](../src/session-engine/stream-bus.ts)); board host resumes auto/AFK boards on **server** boot ([`server/session/board-loader.js`](../server/session/board-loader.js)); renderer skips `board-boot-resume` when the engine is on ([`src/state/board-driver-gate.ts`](../src/state/board-driver-gate.ts)). Client board UI routes through [`src/state/board-command-bridge.ts`](../src/state/board-command-bridge.ts). Engine main-chat loop also exposes in-process `board_*` / `delegate_tasks` tools (headless catalog strips them) so kickoff `board_init` works. **Phase 3 (MIN-361):** sub-agent controller registry / scheduler / watchdog run in the engine ([`src/session-engine/controller-host.ts`](../src/session-engine/controller-host.ts), [`server/session/controller-loader.js`](../server/session/controller-loader.js)) — `initControllerPersistence` + `startWatchdog` once at server boot; controller + main-chat boot call [`bootstrapHeadlessNodeRuntime`](../src/headless/server-context.ts) (patched `fetch` + in-memory `localStorage`) so config/meta loaders used by sub-agents do not throw in Node; live status fans out as ephemeral `SessionState.liveSubAgentRuns` over Phase 0 SSE ([`live-publish.ts`](../src/agents/controller/live-publish.ts)); durable validate still strips the slice from disk, but `writeResource` / `patchResource` reattach it before SSE/rev-cache publish and `adoptExternalSessionWrite` preserves it across client PUTs; clients also keep the prior slice when a remote payload omits the field; commands `spawn_sub_agent` / `cancel_sub_agent`; main-chat loop executes sub-agent tools in-process; thin clients read the SSE slice ([`client-live-mirror.ts`](../src/agents/controller/client-live-mirror.ts)) instead of a local registry. **Phase 4 (MIN-362):** lease deleted; engine default-on; renderer is a read-model + command dispatcher for chat/board/sub-agent driving. Boot generation-resume / incomplete-tool-resume / programmatic `/loop` sends — and secondary callers (sidebar seed kickoff, plan-screen start, board git-setup, onboarding guide, Super Plan stages, fork/replay) — skip the renderer tool loop when the engine is on (mirror + `POST /api/session/commands` / `dispatchSendMessageAndAwaitIdle` instead). Settings / non-driver UI edits may still PATCH/PUT sessions. Opt-out restores legacy renderer driving for emergency single-device use. Engine core: [`server/session/engine.js`](../server/session/engine.js) + [`commands.js`](../server/session/commands.js). Client helpers: [`src/state/session-commands.ts`](../src/state/session-commands.ts), flag probe [`src/state/server-engine-flag.ts`](../src/state/server-engine-flag.ts). **TSX preload:** [`server/session/engine-tsx-hooks.mjs`](../server/session/engine-tsx-hooks.mjs) must **self-register** on `node --import` (CSS/@xterm stubs) so controller + board host can load under Node — exporting `resolve`/`load` alone does not activate hooks. Smoke: [`test/engine/engine-tsx-hooks.test.mjs`](../test/engine/engine-tsx-hooks.test.mjs). `GET /api/session/engine` still requires the per-boot session token (global auth); SPA prefers HTML inject (`__MINNOW_SERVER_ENGINE__`) and falls back via `withSessionToken`. Plan: [`plans/server-session-engine.md`](plans/server-session-engine.md).

**Sessions SQLite (Phase A → C.2):** On first open, legacy `sessions/state.json` is imported once and renamed to `state.json.migrated`. Whole-blob R/W lives in [`server/config/sessions-repo.js`](../server/config/sessions-repo.js) (the persistence seam). Optimistic `rev`/`If-Match` is implemented (Phase 0 above) on both PUT and PATCH paths via [`server/config/store.js`](../server/config/store.js) + [`server/config/middleware.js`](../server/config/middleware.js). `terminalHistory` is server-owned on PUT/PATCH (only `appendTerminalRun` writes it). **PATCH `/api/config/sessions`** accepts `{ baseVersion, chats?, deleteChatIds?, groups?, deleteGroupIds?, scalars?, expectedRev? }` — absent keys mean unchanged; deletes are explicit id lists; dirty chats/groups are full objects. Implemented via `patchResource` → `patchSessionState`. **POST** on the same path is a `sendBeacon` alias for PATCH (beacons cannot PATCH). Headless [`src/headless/persist-chat.ts`](../src/headless/persist-chat.ts) uses PATCH (no GET-splice-PUT). **B.2 SPA flush:** `saveSessionsNow` uses PATCH when `sessionsClientPatchEnabled` (default **ON**) and dirty sets are trusted; full PUT on the first save after load or after a dirty-tracking verifier miss. Dirty sets (`touchChat` / `dirtyChatIds` / `deletedChatIds` / `dirtyGroupIds` / `deletedGroupIds` / `sessionScalarsDirty`) clear only after a successful PATCH/PUT. Shutdown: serialize the delta; if &lt; 60 KiB use `navigator.sendBeacon` (POST alias); else keepalive whole-blob PUT (Fetch keepalive bodies are capped at 64 KiB — large keepalive PUTs were likely silent no-ops). MIN-408: no PATCH/PUT before `sessionsHydratedFromServer`. Dev builds compare chats against a shadow copy at flush and `console.warn` unmarked mutations (forces PUT fallback). A rotating `state.json.backup` mirror flushes on a 5-minute dirty debounce and on `closeSessionsDb()`. Rollback: `MINNOW_SESSIONS_STORE=json`. Export: `POST /api/config/sessions/export-json`. Hot server consumers use indexed SQLite point lookups — see [`plans/sessions-sqlite-migration.md`](plans/sessions-sqlite-migration.md).
...
Browser / Electron (same origin, default :9473)
  ...
  ├─ GET/PUT/PATCH /api/config/*     → ~/.minnow (sessions: SQLite + Phase 0 rev/If-Match)
  ├─ GET  /api/session/stream        → SSE session state (Phase 0)
  ├─ GET  /api/session/engine        → { enabled } (default true; opt-out via env)
  ├─ POST /api/session/commands      → typed commands → 202 { rev } (engine default-on)
...
**Browser-only tools** (`get_datetime`, `calculate`, sub-agent/board tools, mode handoff, `browser_*`) run client-side; `POST /api/tools` returns `Not implemented` for those names. `ask_question` is client-UI in the renderer loop; under the Session Engine it parks server-side and the renderer answers via `answer_question` (see Session Engine above).
...
- **Desktop chat** uses a configurable workspace folder (default `~/.minnow/workspace`); change it from the desktop **Files** drawer (MRU select or folder picker). Persisted in `config.json` as `desktopWorkspace.path`. The file tree reloads on switch; `list_directory` tool-cache scopes include `workspaceRoot` so listings from one desktop folder are not reused for another. **Bidirectional sync:** the Chats rail lists all `appScope: desktop` threads (one list across folders); activating a rail item moves the desktop workspace folder to that chat's `workspacePath` (`alignDesktopWorkspaceToChat`); picking a folder from the workspace select or MRU switches `activeId` to the per-folder remembered/newest desktop chat via `lastActiveChatIdByWorkspace` (`onDesktopWorkspaceChanged` / `resolveActiveDesktopChatIdForWorkspace`). Bootstrap and session-engine SSE reconcile re-assert a desktop-scoped `activeId` when Code/orchestrator threads would otherwise bleed through (`restoreDesktopSessionOnForeground`). Global `resolveActiveDesktopChatId` (folder-independent, `lastActiveChatIdByApp.desktop`) remains for desktop foreground restore — it re-points the folder to match the remembered thread. Legacy Chat app uses `~/.minnow/chats`.
...
Main chat: `POST /api/generations` + `GET .../stream` with replay. Client stores `chat.currentGenerationId`. With Session Engine **on** (default), reload mirrors tokens via [`src/chat/engine-stream-mirror.ts`](../src/chat/engine-stream-mirror.ts) — [`generation-resume.ts`](../src/chat/generation-resume.ts) must not call renderer `runChatTurn` (that would double-drive tools). The mirror paints **content and reasoning** (ThoughtBubble + “Generating response…” / “Thinking…”), registers a [`stream-chat-dom`](../src/tools/stream-chat-dom.ts) remount listener (without joining `streamingChatIds`, so Phase 0 reconcile stays live), and re-attaches after `renderChatFromHistory` wipes live shells on boot / SSE reconcile. Emergency opt-out still uses generation-resume → `runChatTurn`. Stop: [`src/chat/stop-generation.ts`](../src/chat/stop-generation.ts).
...
State: `Chat.orchestratePlanPath`, `ChatGroup.orchestrateBoard`, [`src/ui/orchestrate-board.ts`](../src/ui/orchestrate-board.ts). Global defaults (`autopilot` block in `config.json`): Settings → **Autopilot** ([`src/ui/settings-autopilot.ts`](../src/ui/settings-autopilot.ts)) — emphasis-panel layout matching Agents/Rules (board execution, retries, heartbeat, planner fallback, self-heal). Per-board overrides on the board header. With the Session Engine default-on, board resume runs on **server** boot ([`server/session/board-loader.js`](../server/session/board-loader.js)); renderer [`board-boot-resume.ts`](../src/chat/orchestrate/board-boot-resume.ts) is a no-op unless emergency opt-out (`MINNOW_SERVER_ENGINE=0`).
...
**LSP:** Bundled TS/JS + on-demand language bundles; config `~/.minnow/lsp.json`, defaults `src/lsp/defaults.json`. APIs: `/api/lsp/*`. The app pins `typescript` to **6.x** (not 7.x) because `typescript-language-server` still requires `typescript/lib/tsserver.js`, which TypeScript 7 removed. Resolution also checks the on-disk `node_modules/typescript/lib/tsserver.js` path when package exports block `require.resolve`.
...
[`src/attachments/`](../src/attachments/) — composer chips, max **10 MB**. Images → VLM `image_url` parts when model supports vision. PDF/office → server `read_document` when `npm start`. Shared history/VLM builders live in [`api-content.ts`](../src/attachments/api-content.ts) (renderer loop + Session Engine). With the engine on, composer/`sendProgrammaticChatText` pass attachments on `send_message` (no `MINNOW_SERVER_ENGINE=0` hard-block).
