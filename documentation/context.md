...
**Desktop experts** ([`experts-desktop.ts`](../src/os/experts-desktop.ts)): `expertsIdle` / `expertsActive`; dock **Experts** enters **idle** first — hero shows a grid of specialist buttons (`#desktopExpertsHeroMount`) plus **Open Experts' Lab** / **New expert**; clicking a specialist starts an expert sandbox chat; **Open Lab** promotes to `expertsActive` and mounts `#expertsView` in `#desktopExpertsMount` (full roster/detail/create UI); lab header **Close** (after **New expert**) demotes to idle hero; `#/experts` / `#/app/experts` → `#/desktop` + queue activation; composer placeholder “Message an expert…”; hero copy **Experts' Lab.** + picker subline; `openLab` / create/edit steps open the panel immediately; [`app-host.ts`](../src/os/app-host.ts) preserves hub visibility while desktop experts mode is on; **New chat** from the lab opens expert threads on the **desktop chat** surface; docked composer uses `pointer-events: none` on `.mn-os-composer-dock` with interactive children only so lab **Edit** / **Delete** and chat-row actions stay clickable above the composer reserve; styles in `minnowos-desktop.css` + `experts-hub.css`.
