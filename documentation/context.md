...
Local Whisper STT + Qwen3-TTS with optional provider/browser fallbacks. Plan: [`documentation/plans/built-in-voice-system.md`](plans/built-in-voice-system.md). UI: **Settings → Audio** ([`src/ui/settings-audio.ts`](../src/ui/settings-audio.ts)) for mic/speaker device routing; **Models → Voice** ([`src/ui/models/voice-panel.ts`](../src/ui/models/voice-panel.ts), `#/app/models/voice`) for runtime, catalogs, downloads, and STT/TTS settings. Legacy `#/settings/voice` redirects via [`src/os/router.ts`](../src/os/router.ts) and [`src/ui/settings-page.ts`](../src/ui/settings-page.ts). Composer dictation: [`src/ui/composer-voice.ts`](../src/ui/composer-voice.ts); read-aloud: [`src/ui/voice-controls.ts`](../src/ui/voice-controls.ts) — respects `voice.tts.backend` (`local` | `provider` | `browser`) and `setSinkId` when available.
