...
Browser / Electron (same origin, default :9473)
  ...
  ├─ POST /api/session/lease         → board driver lease (Phase 0)
  ├─ GET  /api/session/engine        → { enabled } (Phase 1 flag probe)
  ├─ POST /api/session/commands      → typed commands → 202 { rev } (Phase 1–2, flag on)
...
State: `Chat.orchestratePlanPath`, `ChatGroup.orchestrateBoard`, [`src/ui/orchestrate-board.ts`](../src/ui/orchestrate-board.ts). Global defaults (`autopilot` block in `config.json`): Settings → **Autopilot** ([`src/ui/settings-autopilot.ts`](../src/ui/settings-autopilot.ts)) — emphasis-panel layout matching Agents/Rules (board execution, retries, heartbeat, planner fallback, self-heal). Per-board overrides on the board header. After reload, [`board-boot-resume.ts`](../src/chat/orchestrate/board-boot-resume.ts) resumes delegation when `autoRunning === true` **only if this client holds the Phase 0 board driver lease**.
