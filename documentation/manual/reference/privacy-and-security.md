...
**Today:** Settings → General → **Agent shell sandbox** (`toolSecurity.shellSandbox`: `off` / `prefer` / `require`, default **off**). Dev flag **`MINNOW_SHELL_SANDBOX=1`** still elevates `off` → `prefer`. On **macOS**, Seatbelt via `sandbox-exec` wraps the argv after shell/WSL resolution. Linux uses the `minnow-sandbox` Landlock helper when packaged; Windows containment needs WSL2+Landlock (Phase 6) — otherwise the adapter reports unavailable honestly and does not pretend WSL alone is a sandbox.

**Behaviour:** Unavailable under `prefer` → Ask strip to run unsandboxed (Allow once / Always allow / Cancel); under `require` → clear error, no silent fallback. AFK/autopilot boards default to **`require`** via `autopilot.shellSandbox` (overridable). Tool results append `[sandboxed: …]` or `[NOT sandboxed: …]` trailers; the UI shows a badge on agent shell tool cards.
