...
| `sessions/sessions.db` | All chats and history (SQLite; HTTP still GET/PUT whole blob). Legacy `state.json` imports once → `state.json.migrated`; rotating `state.json.backup` mirror. |
