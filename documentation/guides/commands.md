...
**Defaults:** worker concurrency `min(16, availableParallelism())`, up to `300` files per `node --test` spawn (also capped by a 24k argv budget in `run-all.mjs`). Paths under `test/ui/`, `test/orchestrate/`, and `test/chat/orchestrate/` always spawn alone with `--test-isolation=process`. Tune with `MINNOW_TEST_CONCURRENCY` / `MINNOW_TEST_BATCH_SIZE`.
