...
**Defaults:** worker concurrency `min(16, availableParallelism())`, up to `300` files per `node --test` spawn (also capped by a 24k argv budget in `run-all.mjs`). Paths under `test/ui/`, `test/orchestrate/`, and `test/chat/orchestrate/` batch together (never with light files) and use `--test-isolation=process` so each file still runs in its own child. Tune with `MINNOW_TEST_CONCURRENCY` / `MINNOW_TEST_BATCH_SIZE`.
