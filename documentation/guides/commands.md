...
**Memory:** every runner preloads [`test/assert-dom-safe.mjs`](../../test/assert-dom-safe.mjs). Without it a *failing* `assert.equal(document.querySelector('.x'), null)` hands a happy-dom node to node:assert, which inspects it at `depth: 1000` and Myers-diffs the result — synchronous, unbounded typed-array allocation that `--max-old-space-size` cannot cap, and enough to freeze a 64 GB workstation from a single test process. The guard compares DOM operands itself and reports a short descriptor (`<section.board-root>`) instead. Do not remove it, and prefer `assert.ok(!el)` over comparing elements when adding assertions.

**Defaults:** worker concurrency `4`, up to `200` files per `node --test` spawn (batching costs no memory — node:test still forks one child per file). A heavy happy-dom + board file peaks near 300 MB, so the suite stays around 1.5 GB. Paths under `test/ui/`, `test/orchestrate/`, and `test/chat/orchestrate/` get their own spawn so a hang is attributable to one file. Tune with `MINNOW_TEST_CONCURRENCY` / `MINNOW_TEST_BATCH_SIZE`, e.g. `MINNOW_TEST_CONCURRENCY=1 npm test` to serialize.
