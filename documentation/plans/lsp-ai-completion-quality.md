...
- [ ] Pass live buffer text and completion context through the LSP API.
- [ ] Wire trigger characters and immediate member completion.
- [ ] Preserve LSP ranking, filtering, preselection, commit characters, and incomplete-list state.
- [ ] Add LSP abort, local filtering, and incomplete-list re-query behavior.
- [ ] Add an agent-scoped saved-disk LSP session and event-driven diagnostic settling.
- [ ] Distinguish confirmed-clean diagnostics from timeout/unavailable outcomes and remove generic tool caching.
- [ ] Add delayed, clean, timeout, real-TypeScript, and disk-vs-editor diagnostic tests.
- [ ] Add the shared LSP/AI arbitration policy.
- [ ] Gate AI requests to meaningful idle edits.
- [ ] Improve AI context with recent edits and bounded LSP signals.
- [ ] Implement cursor-aware output alignment and validation.
- [ ] Correct chat/FIM transport and replace the completion cache.
- [ ] Unify client/server config defaults and validators, update settings guidance, and migrate config safely.
- [ ] Hot-apply AI settings without remounting CodeMirror.
- [ ] Add deterministic fetch, generation, and timer injection seams for integration tests.
- [ ] Add deterministic LSP, AI, coordination, and regression tests.
- [ ] Update [`documentation/context.md`](../context.md) after implementation.
