...
Minnow’s programmatic prompt system is **inspired by** several open products; **no vendor prompts were copied verbatim**. This document records what we adopted vs diverged.
...
**Diverged:** Single concatenated `system` message (not multiple system roles); dual-root overrides under `~/.minnow/prompts/`; Full/Lite/Custom profiles with explicit lite caps; tool list only in the `tool-usage` part.
...
**Diverged:** Minnow uses LM Studio locally (not OpenCode’s CLI stack); `prompt-configs/*.json` schema is Minnow-specific; no agent spawn in Step 04 — only compose-time hooks.
...
## Current Minnow (`constants.ts` / settings)
