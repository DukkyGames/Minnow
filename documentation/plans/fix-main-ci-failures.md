...
- [x] macOS `browser_drive_resize` viewport race: wait for `innerWidth` and dispatch `resize` after CDP metrics override (`a6a2148e` CI run 33931363977)
...
10. **macOS CDP resize race** — `browser_drive_resize` returned as soon as `Emulation.setDeviceMetricsOverride` resolved. On macOS Chrome that can land before `window.innerWidth` updates and without a `resize` event, so the live fixture still showed `width: 1280`. The tool now waits for the inner size and dispatches `resize` before returning.
