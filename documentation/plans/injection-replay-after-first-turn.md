...
- Per-source first-turn budgets still apply (code-map token budget, memory inject cap, documents `maxTotalChars`). Transcript storage still truncates notice bodies at 24k chars.
...
- Raising the 24k transcript storage cap
