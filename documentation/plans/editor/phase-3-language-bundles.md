...
- [x] Pre-bundle pyright, vscode-langservers-extracted (HTML/CSS), yaml, bash, dockerfile, graphql in package.json + `$minnow:` resolver (`prebundled: true` in `bundles.json`)
- [x] Windows npm install uses `npm` + `shell: true` (fixes `spawn EINVAL` from `npm.cmd` + `shell: false`)
...
| Area | Location |
|------|----------|
| Catalog | `src/lsp/bundles.json` (Web, Python, Systems, Scripting, Other) |
