...
- **Four composer modes** — General, Build, Plan (no-destructive guard), Debug — plus **Orchestrate** (opens the Boards surface from the sidebar, not a chat; not in the composer picker), **Super Plan**, **Desktop**, **Email**, and **Onboarding** as non-composer modes. Defined in [`src/chat/modes/registry.ts`](src/chat/modes/registry.ts); prompts in [`src/chat/prompts/modes/`](src/chat/prompts/modes/). Reef mode was removed (MIN-473) — do not reintroduce it.
- **113 built-in tools** across web / utility / files / git / code / agents / browser / lsp ([`src/tools/definitions.ts`](src/tools/definitions.ts)). Includes `issue_*` tools and the read-only `minnow_docs_*` tools (search the shipped user manual under `documentation/manual/` only). Entries with an `appId` (8 calendar/email tools) are filtered out while that app is release-gated or user-disabled (MIN-472), so the shipped catalog is 105. V1 board tools (`board_init` and siblings) were deleted in MIN-715.
...
- **Minnow apps — released:** **Code** (the primary surface), Research, Models, Brain, Scheduler, **Issues**, Settings — all `core` (not user-disableable). Legacy `#/app/chat` redirects to `#/app/code/chat`. **Hidden** (`releaseState: 'hidden'`, MIN-471): Compare, Bench, Experts, Calendar, Email — code stays in tree but is omitted from dock, onboarding, Settings, routes, notifications, and `launch_minnow_app`. Registry: [`src/os/app-registry.ts`](src/os/app-registry.ts).
