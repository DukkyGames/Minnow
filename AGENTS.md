...
Minnow is a Vite + TypeScript SPA chat client for LM Studio and other OpenAI-compatible local providers. It includes seven composer modes (General / Build / Plan / Orchestrate / Research / Reef / Debug) plus a global bug tracker (sidebar **All bugs**, `#/bugs`), 60 built-in tools, sub-agents, `~/.minnow` persistence when `npm start` runs, and a full settings page. See `README.md` for setup/scripts and `documentation/context.md` for architecture details. Gap audit: `documentation/plans/feature-audit-roadmap.md`.
