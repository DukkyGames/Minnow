...
- **Minnow apps — released:** Code, Research, Models, Brain, Scheduler, **Issues**, Settings — all `core` (not user-disableable). Legacy `#/app/chat` redirects to `#/app/code/chat`. **Hidden** (`releaseState: 'hidden'`, MIN-471): Compare, Bench, Experts, Calendar, Email — code stays in tree but is omitted from dock, onboarding, Settings, routes, notifications, and `launch_minnow_app`. Registry: [`src/os/app-registry.ts`](src/os/app-registry.ts).
